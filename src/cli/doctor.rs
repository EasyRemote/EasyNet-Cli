...
fn check_agents() -> Vec<Check> {
    let mut out = Vec::new();
    let daemon_rows = match crate::cli::daemon_agent_view::list_agents() {
        Ok(rows) => rows,
        Err(err) => {
            out.push(Check {
    ...
    let to_check: Vec<(String, crate::cli::daemon_agent_view::AgentRuntimeKind)> =
        if daemon_rows.is_empty() {
            vec![
                (
                    "claude-code".to_string(),
                    crate::cli::daemon_agent_view::AgentRuntimeKind::ClaudeCode,
                ),
                (
                    "codex".to_string(),
                    crate::cli::daemon_agent_view::AgentRuntimeKind::Codex,
                ),
            ]
        } else {
            daemon_rows
