...
// not an identity decision. See `daemon::invocation::admission::usage_quota` for the
...
fn daemon_config_path() -> PathBuf {
    crate::daemon::persistence::daemon_config::default_config_path()
}
...
fn parse_quota(raw: &str) -> anyhow::Result<Option<QuotaView>> {
    ...
    let quota = crate::daemon::persistence::daemon_config::QuotaConfig::from(raw_quota);
    Ok(Some(QuotaView {
...
struct RawQuotaDaemon {
    #[serde(default)]
    quota: Option<crate::daemon::persistence::daemon_config::RawQuotaSection>,
}
...
fn write_doc(path: &Path, doc: &toml_edit::DocumentMut) -> anyhow::Result<()> {
    crate::daemon::persistence::config::atomic_write(path, doc.to_string().as_bytes())?;
    Ok(())
}
