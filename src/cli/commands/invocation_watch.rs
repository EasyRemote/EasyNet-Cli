...
// ==========================================
//
// File: src/cli/invocation_watch.rs
...
pub struct WatchArgs {
    ...
    #[arg(long)]
    pub follow: bool,
    /// Maximum seconds `--follow` waits for a trace to produce ledger rows
    /// or for a non-terminal trace to produce new ledger diffs. This does
    /// not cancel the invocation; it bounds the observer.
    #[arg(
...
impl FollowPolicy {
    fn from_args(args: &WatchArgs) -> Self {
        ...
        Self {
            max_empty_polls,
            max_stale_polls: max_empty_polls,
        }
    }
...
impl FollowPhase {
    ...
    fn observe_progress(&mut self) {
        *self = Self::TrackingLedgerRows { stale_polls: 0 };
    }

    fn observe_stale_ledger_rows(&mut self, policy: FollowPolicy) -> FollowStep {
        match self {
            Self::TrackingLedgerRows { stale_polls } => {
                *stale_polls = stale_polls.saturating_add(1);
                if *stale_polls >= policy.max_stale_polls {
                    return FollowStep {
                        events: vec![stale_trace_timeout_liveness_event()],
            ...
            Self::AwaitingLedgerRows { .. } => {
                *self = Self::TrackingLedgerRows { stale_polls: 1 };
                FollowStep {
                    events: Vec::new(),
                    decision: FollowDecision::Continue,
                }
...
impl FollowEngine {
    ...
    fn observe_with_mission_status(
        ...
        if !nodes.is_empty() {
            ...
            // string of polls with no new event is a bounded liveness
            // failure; follow mode must return control to the operator
            // instead of turning a scheduler bug into an infinite stream.
            if events.is_empty() {
                return Ok(self.phase.observe_stale_ledger_rows(self.policy));
            } else {
...
fn mission_follow_status(trace_id: &str) -> MissionFollowStatus {
    let Ok(summary) = crate::cli::commands::mission_runs::find_run(trace_id) else {
    ...
    let interrupted = summary.meta.status
        == crate::cli::commands::mission_runs::MissionRunStatus::Running
        && !summary.running;
    if interrupted {
        MissionFollowStatus::Interrupted
    } else {
...
fn stale_trace_timeout_liveness_event() -> WatchEvent {
    WatchEvent::Liveness {
        status: "ledger_progress_timeout".to_string(),
        source: "watch_follow_policy".to_string(),
    }
}
...
fn mission_terminal_for_empty_trace(trace_id: &str, nodes: &[Record]) -> Option<WatchEvent> {
    ...
    let summary = crate::cli::commands::mission_runs::find_run(trace_id).ok()?;
    if !summary.meta.status.is_terminal() {
        return None;
    }
...
fn mission_terminal_status(
    status: crate::cli::commands::mission_runs::MissionRunStatus,
) -> &'static str {
    match status {
        crate::cli::commands::mission_runs::MissionRunStatus::Ok => "ok",
        crate::cli::commands::mission_runs::MissionRunStatus::Partial
        | crate::cli::commands::mission_runs::MissionRunStatus::Error => "failed",
        crate::cli::commands::mission_runs::MissionRunStatus::Cancelled => "cancelled",
        crate::cli::commands::mission_runs::MissionRunStatus::Running => "running",
    }
}

fn mission_run_exists(trace_id: &str) -> bool {
    crate::cli::commands::mission_runs::find_run(trace_id).is_ok()
}
...
mod tests {
    ...
    #[test]
    fn follow_engine_stops_stale_nonempty_running_trace_after_timeout() {
        // Once a ledger row exists, follow mode still has an observer-side
        // liveness budget. A stale RUNNING row with no diffs must terminate
        // after that budget so a broken scheduler cannot stream forever.
        ...
        // Budget (2) reached on the second stale poll → liveness timeout.
        let timeout = engine
            .observe_with_mission_status(&nodes, MissionFollowStatus::NotMission)
            .expect("stale trace emits a terminal liveness timeout");
        assert_eq!(timeout.decision, FollowDecision::Stop);
        assert_eq!(
            timeout.events,
            vec![WatchEvent::Liveness {
                status: "ledger_progress_timeout".into(),
                source: "watch_follow_policy".into(),
            }]
        );
    }
    ...
    fn follow_engine_does_not_bound_a_progressing_trace() {
        ...
        for i in 0..5 {
            let request_id = format!("01PROG{i}");
            let ura = crate::core::ura::resource_dot_ura("acme", "alice.invocations", &request_id);
    ...
    fn empty_terminal_mission_trace_emits_terminal_event() {
        ...
        let run_dir = crate::cli::commands::mission_runs::root_dir().join(trace_id);
        std::fs::create_dir_all(&run_dir).expect("run dir");
        let meta = crate::cli::commands::mission_runs::MissionRunMeta {
            name: "zero-node".to_string(),
            trace_id: trace_id.to_string(),
            started_at: "2026-06-23T12:00:00+00:00".to_string(),
            status: crate::cli::commands::mission_runs::MissionRunStatus::Ok,
            ..Default::default()
        };
        std::fs::write(
