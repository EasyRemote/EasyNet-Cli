...
Hello World runtime plugin generated by `easynet plugin init`.
...
"#,
    ...
    fn setup_install_commands(&self) -> &'static str {
        match self.language {
            ...
            PluginTemplateLanguage::Go
            | PluginTemplateLanguage::Rust
            | PluginTemplateLanguage::Java => "make build\neasynet plugin install .",
            PluginTemplateLanguage::Node => "npm install\neasynet plugin install .",
        }
    }
...
mod tests {
    ...
    fn init_hello_plugin_generates_installable_project() {
        ...
        for package_owned_field in [
            ...
            assert!(
                !ability_body.contains(package_owned_field),
        ...
        assert!(
            ...
            "python template should not claim a compiled build step"
        );
        assert!(
            !readme.contains("npm install"),
            "python template should not claim a Node dependency step"
        );
