...
pub use crate::support::platform::output::OutputFormat;
...
impl DiscoverExecutionState {
    ...
    fn record_source_diagnostic(&mut self, value: &Value) {
        ...
        self.diagnostics.push(DiscoverDiagnostic {
            ...
            message: format!(
                "runtime returned {limit} of {available} source candidates before final CLI ranking"
            ),
        });
    }
        ...
        let anchor = match resolve_realm_anchor(&parent_invocation_meta) {
            ...
            RealmAnchorResolution::Unanchored { reason } => {
                self.state.record_diagnostic(DiscoverDiagnostic {
                    scope: scope.to_string(),
                    code: "causal_anchor_unavailable",
                    message: reason,
                });
                return Ok(());
            }
...
#[derive(Debug, Clone)]
enum RealmAnchorResolution {
    Anchored(RealmAnchor),
    Unanchored { reason: String },
}
...
fn receipt_parent_from_anchor(anchor: &Value) -> anyhow::Result<Value> {
...
fn truncate(text: &str, max: usize) -> String {
    crate::cli::commands::abilities::truncate_display(text, max)
}
...
mod tests {
    ...
    /// `crate::core::ura` — never hand-written literals (spec §0.1-8).
    fn agent_row(realm: &str) -> Value {
        json!({
            "qualified_name": crate::core::ura::ability_ura(realm, "user-1", "codex", "chat"),
            "owner": "codex",
            "ability": "chat",
    ...
    #[test]
    fn missing_local_receipt_anchor_remains_unanchored_without_fabricating_parent() {
    ...
    fn device_aggregate_projection_does_not_invent_self_identity() {
        ...
        let private_self = json!({
            "qualified_name": crate::core::ura::ability_ura("acme", "user-1", "codex", "private_chat"),
            "owner": "codex",
