...
pub fn run(invoke_args: InvokeArgs) -> anyhow::Result<()> {
    ...
    // `daemon::invocation::federation_invoke` helper. The path requires the
    ...
    let node_ura: Option<String> = match invoke_args.node.as_deref().map(str::trim) {
        ...
        Some(node) => {
            #[cfg(feature = "axon-pb")]
            {
                Some(crate::daemon::invocation::federation_invoke::parse_node_ura(node)?)
            }
    ...
    let (result, fulfilled_label) = match node_ura.as_deref() {
        ...
        Some(target) => {
            ...
            let value =
                crate::daemon::invocation::federation_invoke::invoke_via_federation_forward_target_with_timeout(
                    &target_call,
                    arguments,
                    caller_ura.as_deref(),
...
impl InvokeAbilityRef {
    ...
    ) -> anyhow::Result<crate::daemon::invocation::federation_invoke::RemoteAbilityInvocationTarget>
    {
        match self.descriptor_ref.as_deref() {
            Some(descriptor_ref) => {
                crate::daemon::invocation::federation_invoke::RemoteAbilityInvocationTarget::from_descriptor_ref(
                    execution_target_ura,
                    descriptor_ref,
                )
            }
            None => {
                crate::daemon::invocation::federation_invoke::RemoteAbilityInvocationTarget::from_ability_ura(
                    execution_target_ura,
