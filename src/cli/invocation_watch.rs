...
fn mission_follow_status(trace_id: &str) -> MissionFollowStatus {
    let Ok(summary) = crate::cli::mission_runs::find_run(trace_id) else {
    ...
    let interrupted = summary.meta.status == crate::cli::mission_runs::MissionRunStatus::Running
        && !summary.running;
    if interrupted {
        MissionFollowStatus::Interrupted
    } else {
...
fn mission_terminal_for_empty_trace(trace_id: &str, nodes: &[Record]) -> Option<WatchEvent> {
    ...
    let summary = crate::cli::mission_runs::find_run(trace_id).ok()?;
    if !summary.meta.status.is_terminal() {
        return None;
    }
...
fn mission_terminal_status(status: crate::cli::mission_runs::MissionRunStatus) -> &'static str {
    match status {
        crate::cli::mission_runs::MissionRunStatus::Ok => "ok",
        crate::cli::mission_runs::MissionRunStatus::Partial
        | crate::cli::mission_runs::MissionRunStatus::Error => "failed",
        crate::cli::mission_runs::MissionRunStatus::Cancelled => "cancelled",
        crate::cli::mission_runs::MissionRunStatus::Running => "running",
    }
}

fn mission_run_exists(trace_id: &str) -> bool {
    crate::cli::mission_runs::find_run(trace_id).is_ok()
}
...
mod tests {
    ...
    fn follow_engine_does_not_bound_a_progressing_trace() {
        ...
        for i in 0..5 {
            ...
            let ura = crate::ura::resource_dot_ura("acme", "alice.invocations", &request_id);
    ...
    fn empty_terminal_mission_trace_emits_terminal_event() {
        ...
        let run_dir = crate::cli::mission_runs::root_dir().join(trace_id);
        std::fs::create_dir_all(&run_dir).expect("run dir");
        let meta = crate::cli::mission_runs::MissionRunMeta {
            name: "zero-node".to_string(),
            trace_id: trace_id.to_string(),
            started_at: "2026-06-23T12:00:00+00:00".to_string(),
            status: crate::cli::mission_runs::MissionRunStatus::Ok,
            ..Default::default()
        };
        std::fs::write(
