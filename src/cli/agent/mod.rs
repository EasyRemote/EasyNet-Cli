...
use crate::cli::daemon_agent_view::{AgentRuntimeKind, DaemonAgentRow};
...
fn run_agent_scoped(tokens: Vec<String>) -> anyhow::Result<()> {
    ...
    crate::cli::agent_new_ability::run_scoped(selector, tail)
}
...
fn resolve_local_daemon_caller_ura() -> Option<String> {
    ...
    let plan = crate::cli::start::build_bootstrap_plan_from(
        &creds.realm,
        &creds.node_id,
        user_id,
        username,
...
pub(super) fn invoke_daemon_agent_list_required(
    ...
    crate::cli::daemon_agent_view::list_agents_with_client(client)
}
...
pub(super) fn daemon_row_agent_type(row: &DaemonAgentRow) -> anyhow::Result<AgentRuntimeKind> {
    crate::cli::daemon_agent_view::agent_kind(row)
}

pub(super) fn daemon_row_root(row: &DaemonAgentRow) -> std::path::PathBuf {
    crate::cli::daemon_agent_view::agent_root(row)
}
