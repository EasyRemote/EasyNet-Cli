...
fn invoke_call_signaling(ability: &str, args: Value) -> anyhow::Result<Value> {
    ...
    if let Ok(creds) = crate::persistence::config::load_credentials() {
        ...
        if !realm.is_empty() && !node_id.is_empty() {
            ...
            let target_call =
                crate::daemon::invocation::federation_invoke::RemoteAbilityInvocationTarget::for_target_owned_selector(
                    &hub_ura, ability,
                )?;
            return crate::daemon::invocation::federation_invoke::invoke_via_federation_forward_target(
                &target_call,
                args,
                Some(&caller_ura),
            )
