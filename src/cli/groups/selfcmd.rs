...
use crate::cli::stop::{self, StopOptions};
...
impl UninstallEnvironment for ProductionUninstallEnvironment {
    fn capture_device_identity(&mut self) -> Option<DeviceIdentity> {
        ...
        Some(DeviceIdentity {
            device_ura: crate::ura::device_ura(&creds.realm, &creds.node_id),
        })
    }
    ...
    fn report_hub_removal(&mut self, identity: &DeviceIdentity) -> HubRemovalReport {
        ...
        #[cfg(feature = "axon-pb")]
        {
            match crate::daemon::invocation::federation_invoke::invoke_federation_revoke(
                &identity.device_ura,
                "self uninstall",
            ) {
