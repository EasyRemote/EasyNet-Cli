...
// or stop.
//
// So `device.agent.start` is "register a new agent row + return its
// canonical URA", and `device.agent.stop` is "remove the row." The
...
fn start_agent_handler(args: Value) -> anyhow::Result<Value> {
    ...
    let agent_type_str = args
        .get("agent_type")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow::anyhow!("device.agent.start: `agent_type` required"))?;
...
fn stop_agent_handler(args: Value) -> anyhow::Result<Value> {
    let name_or_uri = args
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
            anyhow::anyhow!("device.agent.stop: `name_or_uri` (non-empty string) required")
        })?;
...
fn derive_canonical_uri(name: &str) -> String {
    let (realm, user_id) = crate::persistence::local_agents::load()
        .ok()
        .and_then(|f| extract_realm_user_from_uri(&f.host_device_agent_ura))
        .unwrap_or_else(|| ("<unset>".to_string(), "<unset>".to_string()));
    crate::ura::agent_ura(&realm, &user_id, &format!("01LLM-{name}"))
}
...
mod tests {
    ...
    fn start_agent_persists_and_returns_canonical_uri() {
        with_isolated_home(|| {
            ...
            let uri = resp["agent_ura"].as_str().unwrap();
