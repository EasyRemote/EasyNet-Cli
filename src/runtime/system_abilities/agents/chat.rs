...
// =======================================================
//
// File: src/runtime/system_abilities/agents/chat.rs
...
pub(crate) fn build_discover_handler_for(
    ...
    federation_resolver: crate::runtime::system_abilities::agents::discover::SharedDiscoverFederationResolver,
) -> crate::runtime::ability_dispatch::LocalRpcHandler {
    ...
    Arc::new(move |args: Value| {
        ...
        crate::runtime::system_abilities::agents::discover::dispatch(
            &agent_name,
            &provider,
            &dispatch_handle,
...
pub(crate) fn build_invoke_handler_for(
    ...
    let provider: Arc<dyn Fn() -> crate::registry::agents::AgentRegistry + Send + Sync> =
        Arc::new(|| crate::registry::agents::load_agents().unwrap_or_default());
    Arc::new(move |args: Value| {
        crate::runtime::system_abilities::agents::invoke::dispatch(
            &agent_name,
            &provider,
            &dispatch_handle,
            args,
...
pub(crate) fn invoke_direct_with_progress(
    ...
    let files_root = if parsed.attachments.iter().any(AttachmentSpec::is_ura) {
        Some(crate::runtime::system_abilities::resources::files_store::state::root_from_env()?)
    } else {
        None
    };
...
fn materialize_attachments(
    ...
    for (idx, spec) in specs.iter().enumerate() {
        ...
        if crate::runtime::system_abilities::device_control::files::is_blocked_read_path_for_chat(
            path,
        ) {
...
fn materialize_ura_attachment(
    ...
    let sha =
        crate::runtime::system_abilities::resources::files_store::handlers::sha256_from_ura(ura)
            .map_err(|e| anyhow::anyhow!("chat: attachments[{idx}]: {e}"))?;
    let blob = crate::runtime::system_abilities::resources::files_store::state::blob_path(
        files_root, &sha,
    )
...
mod tests {
    ...
    fn ura_attachment_fixture(
        ...
        let ura = crate::runtime::system_abilities::resources::files_store::state::blob_ura(
            "easynet.run",
            "alice",
            &sha,
        );
