...
mod tests {
    ...
    fn plugin_host_surface_reports_index_errors_without_runtime_visibility() {
        ...
        let rows = PluginSurfaceProjector::project_with_daemon(&index, &plan, None, &errors);

        assert_eq!(rows.len(), 1);
        let row = &rows[0];
        assert_eq!(row.package_id, "broken.plugin");
        assert_eq!(row.planned_load_status, "index_error");
        assert!(!row.descriptor_published);
        assert!(!row.runtime_published);
        assert!(!row.invokable);
        assert!(row.error.as_deref().unwrap().contains("hash mismatch"));
    }
}
