    ...
    let agents_json: Vec<serde_json::Value> = registry
        ...
        .map(|(name, e)| {
            let mut skills: Vec<serde_json::Value> =
                crate::daemon::execution::mission::agent_ability_specs::abilities_for(name, e)
                    .iter()
...
fn test_entry_with_agent_directory(
    ...
    let mut entry = AgentEntry::new(agent_type, model.map(String::from));
...
mod tests {
    ...
    fn adversarial_agent_name_cannot_be_registered_so_skills_shape_stays_safe() {
        ...
        use crate::daemon::persistence::agent_registry::validate_agent_name;
