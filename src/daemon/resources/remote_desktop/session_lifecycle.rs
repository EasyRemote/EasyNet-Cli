...
// ===============================================
//
// File: src/daemon/resources/remote_desktop/session_lifecycle.rs
...
use crate::daemon::resources::remote_desktop::session::{now_ms, RemoteDesktopSession};
use crate::daemon::resources::remote_desktop::session_access::{
    ensure_session_control_identity, ensure_session_resource_identity,
};

pub(in crate::daemon::resources::remote_desktop) fn ensure_session_access(
...
pub(in crate::daemon::resources::remote_desktop) fn ensure_session_control_access(
    plugin: &RemoteDesktopPlugin,
    ability: &'static str,
    env: &EnvelopeContext,
    args: &Value,
...
pub(in crate::daemon::resources::remote_desktop) fn stop_direct_webrtc_endpoint(
    plugin: &RemoteDesktopPlugin,
    session_id: &str,
) {
...
pub(in crate::daemon::resources::remote_desktop) fn stop_session_transports(
    plugin: &RemoteDesktopPlugin,
    session_id: &str,
    session: &mut RemoteDesktopSession,
) {
...
pub(in crate::daemon::resources::remote_desktop) fn spawn_session_lease_watchdog(
...
pub(in crate::daemon::resources::remote_desktop) fn expire_session_from_watchdog(
    plugin: &RemoteDesktopPlugin,
    session_id: &str,
...
pub(in crate::daemon::resources::remote_desktop) fn prune_inactive_sessions(
    plugin: &RemoteDesktopPlugin,
...
mod tests {
    ...
    use crate::daemon::resources::remote_desktop::session::{
    ...
    use crate::daemon::resources::remote_desktop::test_support::{
    ...
    fn expired_session_rejects_non_end_operations() {
        ...
        let created = crate::daemon::resources::remote_desktop::handlers::create_session::handle(
            Arc::clone(&plugin),
            env_for(&ura),
            json!({
        ...
        let err = crate::daemon::resources::remote_desktop::handlers::show_session::handle(
            Arc::clone(&plugin),
            env_for(&ura),
        ...
        let ended = crate::daemon::resources::remote_desktop::handlers::end_session::handle(
            Arc::clone(&plugin),
            env_for(&ura),
            json!({"session_id": "rd-expired-test", "session_token": token.clone()}),
        )
