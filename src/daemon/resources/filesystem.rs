...
fn parse_filesystem_resource_ura(resource_ura: &str) -> Result<FilesystemResourceParts> {
    let parsed = crate::ura::parse_ura(resource_ura)
    ...
    Ok(FilesystemResourceParts {
        owner_ura: crate::ura::device_ura(&parsed.realm, device_id),
...
fn virtual_root_path(virtual_root: &str) -> Option<PathBuf> {
    match virtual_root {
        ...
        VIRTUAL_ROOT_HOME => Some(crate::persistence::config::home_dir()),
        _ => None,
    }
}
