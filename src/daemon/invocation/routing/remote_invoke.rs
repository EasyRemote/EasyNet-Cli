...
//    be a canonical Axon Device or Authority URA; non-canonical inputs
...
/// Validate a `--node` argument as a canonical Axon Device or Authority URA.
...
/// Device or Authority URA, never a hosted Agent/User/Ability/Resource.
/// Authority targets must be generated by the CLI `hub_ura(realm)` facade,
...
pub fn parse_node_ura(node: &str) -> anyhow::Result<String> {
    ...
    let identity = crate::core::identity::RuntimeIdentityUra::parse(trimmed).map_err(|err| {
        anyhow::anyhow!(
            "--node `{trimmed}` is not a canonical Axon Device or Authority URA: {err}. \
    ...
    match identity.kind() {
        ...
        other => bail!(
            "--node `{trimmed}` is a canonical Axon URA, but not a Device or Authority target. \
             Got kind={other}. \
             {URA_DISCOVERY_HINT}"
        ),
    }
...
fn validate_remote_target_ura(target_ura: &str) -> anyhow::Result<()> {
    ...
    match identity.kind() {
        ...
        other => {
            bail!("target URA `{target_ura}` must identify a Device or Authority, got kind={other}")
        }
    }
}
