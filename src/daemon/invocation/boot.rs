...
use crate::daemon::invocation::local_session_dispatcher::LocalAxonSessionDispatcher;
...
pub fn start_daemon_invocation_transport(
    ...
    let hub_signing_seed =
        crate::daemon::invocation::peer_envelope_signer::read_hub_identity_seed(config.realm())
            .ok();
    ...
    let escalation_state = if matches!(config.mode(), DaemonMode::Device) {
        let correlation =
            crate::daemon::invocation::session_escalation::EscalationCorrelation::new();
        let outbox = crate::daemon::invocation::session_escalation::SharedSessionOutbox::new();
        let handle = std::sync::Arc::new(
            crate::daemon::invocation::session_escalation::spawn_escalation_consumer_with_outbox(
        ...
        let device_trust_sync = Arc::new(
            crate::daemon::invocation::device_trust_sync::DeviceTrustSync::new(
                config.realm().to_string(),
                trust_anchor_path.clone(),
                trust_anchor_cell.clone(),
                Arc::clone(&handle),
    ...
    if matches!(config.mode(), DaemonMode::Device) {
            ...
            (config.hub_endpoint().map(str::to_string), daemon_identity)
        {
            ...
            let user_trust_sync = crate::daemon::invocation::session_initiator::UserTrustSync {
...
struct SessionHotAgentAdvertiser {
    escalation: Arc<crate::daemon::invocation::session_escalation::SessionEscalationHandle>,
    caller_ura: String,
...
struct DeviceEscalationState {
    ...
    outbox: crate::daemon::invocation::session_escalation::SharedSessionOutbox,
    device_trust_sync: Arc<crate::daemon::invocation::device_trust_sync::DeviceTrustSync>,
}
...
fn spawn_session_supervisor(config: SessionSupervisorConfig) -> anyhow::Result<SessionShutdown> {
    ...
    tokio::spawn(run_session_supervisor(
        crate::daemon::invocation::session_initiator::SessionSupervisorRunConfig {
            hub_endpoint,
