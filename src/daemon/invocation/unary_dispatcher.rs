...
use crate::daemon::invocation::list_user_pubkeys::handle as handle_list_user_pubkeys;
use crate::daemon::invocation::peer_envelope_signer::PeerInvokeRequest;
use crate::daemon::invocation::register_device_pubkey::handle as handle_register_device_pubkey;
use crate::daemon::invocation::register_device_pubkey::parse_register_pubkey_intent;
use crate::daemon::invocation::revoke_user_pubkey::{
    handle_with_outcome as handle_revoke_user_pubkey_with_outcome, parse_revoke_user_pubkey_intent,
};
use crate::daemon::invocation::route_resolver::{
...
impl UnaryDispatcher {
    ...
    pub(crate) fn dispatch_register_device_pubkey(
        ...
        let write_gate = crate::daemon::invocation::identity_write_gate::IdentityWriteGate::new(
    ...
    pub(crate) async fn dispatch_local_presence_forward_invoke(
        ...
        selected_route: &SelectedInvokeRoute,
        correlation_call_id: &str,
        origin_caller: Option<&crate::daemon::invocation::origin_caller::OriginCallerClaim>,
        caller_envelope: Option<&Envelope>,
    ) -> Result<Response<InvokeResponse>, Status> {
    ...
    pub(crate) async fn dispatch_self_targeted_invoke_remote(
        ...
        origin_claim: Option<&crate::daemon::invocation::origin_caller::OriginCallerClaim>,
    ...
    async fn escalate_forward_invoke(
        &self,
        handle: &std::sync::Arc<
            crate::daemon::invocation::session_escalation::SessionEscalationHandle,
        >,
        arguments: &[u8],
    ) -> Result<Response<InvokeResponse>, Status> {
        let outcome = handle
