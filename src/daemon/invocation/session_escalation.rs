...
use crate::daemon::invocation::session_initiator::{SessionUpSender, SESSION_STREAM_ID};
    ...
    async fn escalate_resolves_when_correlation_completes() {
        ...
        tokio::spawn(async move {
            while let Some(frame) = up_rx.recv().await {
                ...
                let dispatch: crate::daemon::invocation::invoke_remote_initiator::SessionDispatch =
                    serde_json::from_slice(&chunk.data).expect("decode");
                if let crate::daemon::invocation::invoke_remote_initiator::SessionDispatch::Request {
                    call_id, ..
                } = dispatch
                {
