    ...
    vec![
        ...
        tool(
            ...
            "Invoke an ability by tool_name. Omit `node_id` to auto-route: the \
             ...
             Set `trace=true` to receive a per-invocation PhaseReceipt alongside \
             the result (`{result, trace: {phase: invoke, status, duration_ms, \
             …}}`); omit or pass `false` for the raw result (default, \
             backwards-compatible shape).",
            &[
                ("node_id", s(), false),
                ("ability", s(), true),
                ("arguments", json!({"type": "object"}), false),
                ("trace", json!({"type": "boolean"}), false),
            ],
        ),
        // ── Orchestration ──────────────────────────────────────────────────
        tool(
...
mod tests {
    ...
    #[test]
    fn invoke_ability_advertises_optional_trace_boolean() {
        // `trace` is the opt-in switch for `PhaseReceipt` telemetry —
        // advertised on the schema so an LLM that wants a receipt
        // knows which property to set, and required=false so existing
        // callers (which never sent `trace`) keep working unchanged.
        // Pinned so a future "let me bury the trace flag somewhere
        // else" refactor trips a visible test instead of silently
        // breaking agent-side discoverability.
        let specs = base_tool_specs();
        let s = find_spec(&specs, "invoke_ability");
        let props = s
            .get("inputSchema")
            ...
            .expect("invoke_ability spec must carry an object inputSchema");
        let trace = props
            .get("trace")
            .expect("invoke_ability must advertise a `trace` property");
        assert_eq!(
            trace.get("type").and_then(Value::as_str),
            Some("boolean"),
            "`trace` must be declared as a boolean, got {trace}"
        );
        assert!(
            !required_set(s).contains("trace"),
            "`trace` must stay optional so non-tracing callers are unchanged"
        );
    }
