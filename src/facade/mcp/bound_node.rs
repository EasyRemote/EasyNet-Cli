...
mod tests {
    ...
    /// Agent-ability names (shape `<agent>.<verb>`) are local-dispatch
    /// only — they never touch the bridge, so a bound-node patch
    /// would be semantically wrong (the ability targets a local
    /// subprocess, not a remote device). The membership list
    /// `NODE_SCOPED_TOOLS` is flat-name only by construction and
    /// cannot contain a dot-name, but pin the property so a future
    /// "I'll add `agent.chat` to the scoped list" would trip this
    /// test with an explanatory name.
    #[test]
    fn agent_ability_names_are_never_scoped_even_under_lock() {
        for ability in ["claude.chat", "codex.chat", "custom.voice"] {
            let result = apply_args_patch(
                Some("node-x"),
                true,
                ability,
            ...
            assert!(
                !result.contains_key("node_id"),
                "bound-node lock must not inject node_id into agent ability {ability:?}"
            );
            // The original arg must survive untouched — a subtle bug
            // would be the patcher *consuming* prompt while not
            // adding node_id.
            assert_eq!(result.get("prompt").and_then(Value::as_str), Some("hi"));
        }
    }

    /// The static-shape pin: no name in `NODE_SCOPED_TOOLS` contains
    /// a `.`, so agent abilities are structurally excluded even if
    /// someone forgot the test above.
    #[test]
    fn node_scoped_tool_names_are_flat_identifiers() {
        for name in NODE_SCOPED_TOOLS {
            assert!(
                !name.contains('.'),
                "node-scoped tool name {name:?} contains `.` — this would \
                 collide with the agent-ability name shape"
            );
        }
    }
