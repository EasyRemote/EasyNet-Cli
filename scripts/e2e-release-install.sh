#!/usr/bin/env bash
# e2e-release-install.sh — sandbox-mode replay of `install.sh` against
# a Phase-A tarball.
#
# What this proves
# ----------------
# The release tarball + the installer logic together produce the
# binary layout that `easynet start` / `easynet device join` /
...
#   2. install.sh fetches the tarball from
...
#
# The "sandbox install" produces the same on-disk shape `install.sh`
# would; the contract being tested is "the tarball + the install
# rules" not "the curl → sudo sh ergonomics". Those are tested in
# `scripts/dev-host-e2e.sh` where it matters.
#
# Usage
# -----
#   scripts/e2e-release-install.sh \
#     [--tarball <path>] \
#     [--prefix <path>] \
#     [--keep-prefix]
#
...
if [ -z "$tarball" ]; then
    ...
    if [ -z "$tarball" ]; then
        ...
        echo "  Run scripts/build-release-tarball.sh first." >&2
        exit 1
    fi
fi
if [ ! -f "$tarball" ]; then
...
# Detect platform with the same matrix install.sh::detect_platform
...
cat > "$env_file" <<EOF
# Sourced by e2e-release-flow.sh. Mirrors what install.sh appends
