...
# ──────────────────────────────────────────────────────────
echo
echo "Case 5: MCP inside src/runtime/agents/mcp.rs is allowed"
mkdir -p "$FIXTURE_ROOT/src/runtime/agents"
# Remove the violating file from Case 4 first.
rm "$FIXTURE_ROOT/src/runtime/mcp_in_kernel.rs"
# Add a file under the allowed path.
cat > "$FIXTURE_ROOT/src/runtime/agents/mcp.rs" << 'EOF'
// mcp-profile Agent implementation — MCP keyword expected here.
fn handle_mcp_request() {}
EOF

output="$(run_conformance baseline)"
# The rule "MCP keyword in CLI src" excludes runtime/agents/mcp.rs.
...
if echo "$mcp_line" | grep -qF "[ ok ]"; then
  printf "  [PASS] %-60s\n" "Case 5 — MCP allowed inside agents/mcp.rs"
else
...
# ──────────────────────────────────────────────────────────
# Case 7: facade/mcp/ legacy directory presence detected.
# ──────────────────────────────────────────────────────────
echo
echo "Case 7: facade/mcp/ legacy directory presence"
mkdir -p "$FIXTURE_ROOT/src/facade/mcp"
output="$(run_conformance baseline)"
if echo "$output" | grep -qE "WARN.*facade/mcp/ legacy directory.*exists"; then
  printf "  [PASS] %-60s\n" "Case 7 — facade/mcp/ presence detected"
else
  printf "  [FAIL] %-60s\n" "Case 7 — facade/mcp/ presence not detected"
  assertions_failed=$((assertions_failed + 1))
fi
assertions_run=$((assertions_run + 1))
