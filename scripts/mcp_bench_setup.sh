...
#   2. Runs mcp-bench's own mcp_servers/install.sh — that script
...
INSTALL_SH="$MCP_BENCH_DIR/mcp_servers/install.sh"
if [[ "$SKIP_INSTALL_SH" == "1" ]]; then
  echo "[mcp_bench_setup] SKIP_INSTALL_SH=1 — skipping 28-server install"
elif [[ -x "$INSTALL_SH" ]]; then
  echo "[mcp_bench_setup] running mcp-bench install.sh (this can take a while)"
  ( cd "$MCP_BENCH_DIR/mcp_servers" && ./install.sh )
else
