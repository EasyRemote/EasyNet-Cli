...
if rg -n 'fn chat_ability\(|fn ensure_chat_manifest\(|fn abilities_for_publication\(|default_chat_manifest\(|agents_root\(\)\.join|falling back to synth|fallback.*synth|entry_without_root_path_falls_back|lazy migration|recreate chat\.ability' "$TARGET"; then
  fail "mission::agent_ability_specs must be manifest-only; publication consumes committed live descriptors, never synthetic chat/root fallbacks"
fi
