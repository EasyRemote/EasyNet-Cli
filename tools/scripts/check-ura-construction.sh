...
# construction ban per repo, the facade (src/core/ura/mod.rs) is the whitelist.
#
...
hits=$(printf '%s\n' "$hits" | grep -v '^$' || true)
if [ -n "$hits" ]; then
    echo "raw URA construction outside src/core/ura/mod.rs (use the facade builders):" >&2
    printf '%s\n' "$hits" >&2
    exit 1
fi
