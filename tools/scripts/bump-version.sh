...
#
# Usage:
#   ./tools/scripts/bump-version.sh              # auto-detect via `tide mark --local-only`
#   ./tools/scripts/bump-version.sh 1.2.3        # use an explicit version
#   ./tools/scripts/bump-version.sh --dry-run    # show what would happen without changing files
# ──────────────────────────────────────────────────────────────────────
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
VERSION_FILE="${ROOT_DIR}/VERSION"
UPDATE_SCRIPT="${ROOT_DIR}/tools/scripts/update-project-version.sh"
