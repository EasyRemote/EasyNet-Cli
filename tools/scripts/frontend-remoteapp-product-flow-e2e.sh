...
DEFAULT_BROWSER_LIFECYCLE_RUNNER="$FRONTEND_ROOT/scripts/remoteapp-browser-lifecycle.mjs"
...
run_frontend_browser_lifecycle() {
  ...
  elif [[ -n "$BROWSER_LIFECYCLE_FRONTEND_URL" && -f "$DEFAULT_BROWSER_LIFECYCLE_RUNNER" ]]; then
    local frontend_root_quoted
    printf -v frontend_root_quoted '%q' "$FRONTEND_ROOT"
    args+=("--runner-cmd" "cd $frontend_root_quoted && npm run e2e:remoteapp-browser" "--frontend-url" "$BROWSER_LIFECYCLE_FRONTEND_URL")
  else
