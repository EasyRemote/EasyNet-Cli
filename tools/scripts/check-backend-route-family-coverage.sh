...
REQUIRED = {
    ...
    "openai_compatibility": {
        ...
        "sdk_primitives": ["Addressing", "RuntimeClient", "StreamHandle"],
        "backend_profile_refs": ["compatibility", "wrappers"],
    },
    "receipts_history_metrics": {
        "route_family": "Call history, receipts, failure location, metrics",
        "sdk_primitives": ["RuntimeClient"],
        "backend_profile_refs": ["receipt", "events"],
    },
    "federation_peer_hubs_remote_devices": {
        "route_family": "Federation, peer hubs, remote devices",
        "sdk_primitives": ["Addressing", "RuntimeClient", "BidiSession"],
        "backend_profile_refs": ["directory", "admin"],
    },
}
...
for family in families:
    ...
    sdk_primitives = require_string_list(family.get("sdk_primitives"), f"{family_id}.sdk_primitives")
    if sdk_primitives != required["sdk_primitives"]:
        fail(f"sdk_primitives_mismatch:{family_id}")

    backend_profile_refs = require_string_list(family.get("backend_profile_refs"), f"{family_id}.backend_profile_refs")
    if backend_profile_refs != required["backend_profile_refs"]:
        fail(f"backend_profile_refs_mismatch:{family_id}")
...
with_profile_ref_mismatch["families"][0]["backend_profile_refs"] = ["runtime_core"]
...
PY
  ...
  grep -Fq "backend_profile_refs_mismatch" "$tmp/profile_refs.out"
