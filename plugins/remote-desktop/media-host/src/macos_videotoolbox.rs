...
// - Plugin-private RemoteApp media-host codec adapter (macOS only).
//
...
use objc2_video_toolbox::{
    ...
    kVTCompressionPropertyKey_MaxFrameDelayCount, kVTCompressionPropertyKey_MaxH264SliceBytes,
...
impl EncoderSession {
    ...
    pub fn encode(
        ...
        let force_keyframe = self.force_next_keyframe.swap(false, Ordering::AcqRel);
        let force_value = force_keyframe.then(|| CFBoolean::new(true));
        let frame_properties: Option<CFRetained<CFDictionary<CFString, CFBoolean>>> =
            force_value.as_ref().map(|value| {
        ...
        let status = unsafe {
            self.session.encode_frame(
                ...
                pts,
                duration,
                frame_properties.as_ref().map(|properties| {
                    let typed: &CFDictionary<CFString, CFBoolean> = properties.as_ref();
                    typed.as_ref()
                }),
                std::ptr::null_mut(),
                &mut info_flags,
        ...
        if status != 0 {
            if force_keyframe {
                self.force_next_keyframe.store(true, Ordering::Release);
            }
            release_in_flight(&self.in_flight);
...
impl EncodedAccessUnitQueue {
    fn new(capacity: usize, force_next_keyframe: Arc<AtomicBool>) -> Self {
        Self {
    ...
    /// Keep only complete H.264 dependency chains.
    ///
    /// Once an encoded delta access unit is dropped, later deltas may depend on
    /// the missing reference and are therefore not transportable. The queue
    /// enters `awaiting_keyframe`, drops deltas, and asks VideoToolbox for a new
    /// IDR. A keyframe atomically starts the next decodable generation.
    fn push_dependency_safe(&self, unit: EncodedAccessUnit) {
        let Ok(mut q) = self.inner.lock() else {
            return;
        };
        ...
        if self.awaiting_keyframe.load(Ordering::Acquire) {
            ...
            let discarded = q.len() as u64;
            q.clear();
            self.dropped.fetch_add(discarded, Ordering::Relaxed);
            self.awaiting_keyframe.store(false, Ordering::Release);
            q.push_back(unit);
            return;
        }
        ...
        if q.len() >= self.capacity {
            ...
            if unit.is_keyframe {
                ...
                self.dropped.fetch_add(1, Ordering::Relaxed);
                self.awaiting_keyframe.store(true, Ordering::Release);
                self.force_next_keyframe.store(true, Ordering::Release);
            }
            return;
        }
    ...
    fn pop_front(&self) -> Option<EncodedAccessUnit> {
        let Ok(mut q) = self.inner.lock() else {
            return None;
        };
        q.pop_front()
...
impl VideoToolboxEncoder {
    pub fn new_with_wakeup_and_limits(
        width: i32,
        height: i32,
        bitrate_kbps: u32,
        ...
        let force_next_keyframe = Arc::new(AtomicBool::new(false));
        let queue = Arc::new(EncodedAccessUnitQueue::new(
            output_queue_capacity.min(ENCODED_OUTPUT_QUEUE_CAPACITY),
            Arc::clone(&force_next_keyframe),
        ));
    ...
    pub fn session(&self) -> EncoderSession {
        EncoderSession {
            ...
            force_next_keyframe: Arc::clone(&self.force_next_keyframe),
        }
    }

    /// Pop one encoded access unit without creating a second daemon-side
    /// staging queue. The codec queue remains the single bounded source of
    /// truth for native encoder backpressure.
    pub fn poll_one(&self) -> Option<EncodedAccessUnit> {
    ...
    /// Request an independently decodable access unit on the next admitted
    /// capture frame. Multiple requests coalesce into one pending IDR.
...
unsafe extern "C-unwind" fn output_callback(
    ...
    ctx.queue.push_dependency_safe(unit);
...
unsafe fn access_unit_from_sample(
    ...
    let encoded_len = unsafe { sample.data_buffer()?.data_length() };
    if encoded_len == 0 {
        return None;
    }
    // One owned Annex-B access unit crosses the encoder callback boundary.
    // Parameter sets and AVCC payload are appended directly into this buffer;
    // no whole-access-unit intermediate Vec is constructed.
    let mut annexb = Vec::with_capacity(encoded_len.saturating_add(256));
    if is_keyframe {
        let _ = unsafe { append_parameter_sets_annexb(sample, &mut annexb) };
    }
    unsafe { append_avcc_block_to_annexb(sample, &mut annexb)? };
    if annexb.is_empty() {
        return None;
    }
...
/// Append one length-prefixed AVCC block buffer as Annex-B NALs.
///
/// VideoToolbox normally exposes a contiguous CMBlockBuffer. That common path
/// is borrowed for the duration of this callback and copied exactly once while
/// replacing AVCC lengths with Annex-B start codes. A non-contiguous CoreMedia
/// block uses one bounded temporary because `CMBlockBufferCopyDataBytes` is the
/// only safe way to linearize it.
unsafe fn append_avcc_block_to_annexb(sample: &CMSampleBuffer, out: &mut Vec<u8>) -> Option<()> {
    ...
    let status =
        unsafe { block.data_pointer(0, &mut length_at_offset, &mut total_length, &mut pointer) };
    if status == 0 && !pointer.is_null() && length_at_offset >= len && total_length >= len {
        let data = unsafe { std::slice::from_raw_parts(pointer.cast::<u8>(), len) };
        append_avcc_bytes(data, out);
        return Some(());
    }

    let mut data = vec![0_u8; len];
    let destination = NonNull::new(data.as_mut_ptr().cast::<c_void>())?;
    if unsafe { block.copy_data_bytes(0, len, destination) } != 0 {
        return None;
    }
    append_avcc_bytes(&data, out);
    Some(())
...
fn append_avcc_bytes(data: &[u8], out: &mut Vec<u8>) {
    let mut i = 0usize;
    while i + 4 <= data.len() {
...
unsafe fn append_parameter_sets_annexb(sample: &CMSampleBuffer, out: &mut Vec<u8>) -> Option<()> {
...
mod tests {
    ...
    #[test]
    fn encoded_queue_restarts_at_keyframe_after_dependency_break() {
        let force_keyframe = Arc::new(AtomicBool::new(false));
        let q = EncodedAccessUnitQueue::new(2, Arc::clone(&force_keyframe));
        q.push_dependency_safe(unit(1, true));
        q.push_dependency_safe(unit(2, false));
        q.push_dependency_safe(unit(3, false));

        assert!(force_keyframe.load(Ordering::Acquire));
        q.push_dependency_safe(unit(4, false));
        q.push_dependency_safe(unit(5, true));
        q.push_dependency_safe(unit(6, false));

        let mut drained = Vec::new();
        while let Some(unit) = q.pop_front() {
            drained.push(unit);
        }
    ...
    fn rewrite_avcc_for_test(data: &[u8]) -> Vec<u8> {
        let mut out = Vec::new();
        append_avcc_bytes(data, &mut out);
        out
    }
