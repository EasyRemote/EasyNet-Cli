...
// ==========================================
//
// File: plugins/remote-desktop/src/session_lease.rs
...
#[derive(Debug, Clone)]
pub(in crate::daemon::plugins::remote_desktop) struct RemoteDesktopLease {
    created_at_ms: u64,
    updated_at_ms: u64,
    expires_at_ms: u64,
}
...
impl RemoteDesktopLease {
    ...
    pub(in crate::daemon::plugins::remote_desktop) fn new(now_ms: u64, ttl_ms: u64) -> Self {
        Self {
    ...
    pub(in crate::daemon::plugins::remote_desktop) fn expires_at_ms(&self) -> u64 {
        self.expires_at_ms
    }

    /// Whether the lease has elapsed at `now_ms`.
    pub(in crate::daemon::plugins::remote_desktop) fn is_expired_at(&self, now_ms: u64) -> bool {
        now_ms > self.expires_at_ms
    }

    /// Mark a non-lease session mutation at `now_ms`.
    pub(in crate::daemon::plugins::remote_desktop) fn touch(&mut self, now_ms: u64) {
        self.updated_at_ms = now_ms;
    }

    /// Refresh the lease and return the new expiry timestamp.
    pub(in crate::daemon::plugins::remote_desktop) fn refresh(
        &mut self,
        now_ms: u64,
        ttl_ms: u64,
    ) -> u64 {
    ...
    #[cfg(test)]
    pub(in crate::daemon::plugins::remote_desktop) fn set_expires_at_for_test(
        &mut self,
        expires_at_ms: u64,
    ) {
