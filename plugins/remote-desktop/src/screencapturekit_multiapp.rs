...
// =========================================================
//
// File: plugins/remote-desktop/src/screencapturekit_multiapp.rs
...
use crate::daemon::plugins::remote_desktop::screencapturekit_capture::{CapturedFrame, FrameSink};
use crate::daemon::plugins::remote_desktop::target::{
    AppSurfaceLayoutProof, RemoteAppTargetError, TargetGeometry, TargetResolutionError,
};

const MAX_MULTI_APP_WINDOWS: usize = 32;
const MAX_MULTI_APP_PIXELS: u64 = 33_177_600;

pub(in crate::daemon::plugins::remote_desktop) struct MultiAppSurfaceTarget {
    ...
    native_width: usize,
    native_height: usize,
    surface_layout_proof: AppSurfaceLayoutProof,
}
...
pub(in crate::daemon::plugins::remote_desktop) struct ScaledAppWindowSurface {
    pub window_id: u64,
...
impl MultiAppSurfaceTarget {
    pub fn from_windows(
        ...
        let mut proof_geometries = Vec::with_capacity(windows.len());
        for (front_to_back_index, window) in windows.into_iter().enumerate() {
            ...
            proof_geometries.push((
                unsafe { window.windowID() as u64 },
                TargetGeometry {
                    x: Some(frame.origin.x),
                    y: Some(frame.origin.y),
                    width: Some(frame.size.width),
                    height: Some(frame.size.height),
                },
            ));
        }
        let logical_width = finite_dimension(max_x - min_x).ok_or_else(|| {
            RemoteAppTargetError::new(
                ability,
                TargetResolutionError::TargetMetadataIncomplete,
                "macOS application surface union width is invalid",
            )
        })?;
        let logical_height = finite_dimension(max_y - min_y).ok_or_else(|| {
            RemoteAppTargetError::new(
                ability,
                TargetResolutionError::TargetMetadataIncomplete,
                "macOS application surface union height is invalid",
            )
        })?;
        let native_width = scaled_native_dimension(logical_width, maximum_point_pixel_scale)
            .ok_or_else(|| {
                RemoteAppTargetError::new(
                    ability,
                    TargetResolutionError::TargetMetadataIncomplete,
                    "macOS application surface native pixel width is invalid",
                )
            })?;
        let native_height = scaled_native_dimension(logical_height, maximum_point_pixel_scale)
            .ok_or_else(|| {
                RemoteAppTargetError::new(
                    ability,
                    TargetResolutionError::TargetMetadataIncomplete,
                    "macOS application surface native pixel height is invalid",
                )
            })?;
        ensure_canvas_bound(ability, native_width, native_height)?;
        let surface_layout_proof = AppSurfaceLayoutProof::from_front_to_back_geometries(
            proof_geometries
                .iter()
        ...
        .ok_or_else(|| {
            RemoteAppTargetError::new(
                ability,
                TargetResolutionError::TargetMetadataIncomplete,
                "macOS application surface layout proof is invalid",
            )
        })?;
        Ok(Self {
            surfaces,
    ...
    pub fn surface_layout_proof(&self) -> &AppSurfaceLayoutProof {
        &self.surface_layout_proof
    }
    ...
    pub fn scale_to(
        ...
        width: usize,
        height: usize,
    ) -> Result<Vec<ScaledAppWindowSurface>, RemoteAppTargetError> {
        ensure_canvas_bound(ability, width, height)?;
        ...
        self.surfaces
            ...
            .map(|surface| {
                ...
                let offset_x = usize::try_from(offset_x as i128).map_err(|_| {
                    invalid_layout(ability, surface.window_id, "negative scaled x offset")
                })?;
                let offset_y = usize::try_from(offset_y as i128).map_err(|_| {
                    invalid_layout(ability, surface.window_id, "negative scaled y offset")
                })?;
                let surface_width = usize::try_from(surface_width as u128).map_err(|_| {
                    invalid_layout(ability, surface.window_id, "scaled width out of range")
                })?;
                let surface_height = usize::try_from(surface_height as u128).map_err(|_| {
                    invalid_layout(ability, surface.window_id, "scaled height out of range")
                })?;
...
pub(in crate::daemon::plugins::remote_desktop) struct MultiAppSurfaceCompositor {
...
impl MultiAppSurfaceCompositor {
    ...
    pub fn accept(&self, surface_index: usize, frame: CapturedFrame) -> anyhow::Result<()> {
...
fn invalid_layout(ability: &'static str, window_id: u64, detail: &str) -> RemoteAppTargetError {
    RemoteAppTargetError::new(
        ability,
        TargetResolutionError::TargetMetadataIncomplete,
        format!("committed window {window_id} has invalid multi-app layout: {detail}"),
    )
}
