...
\begin{itemize}[leftmargin=2em,itemsep=0.3em]
    ...
    receipts.
    \item \textbf{DeviceAgent} --- control-plane identity projection for a
    device. It advertises descriptors; it is not the ultimate authority
    source.
...
\texttt{DeviceAgent advertises AbilityDescriptor}; it does not formally own
Ability. The owner/accountability root is the \texttt{device\_ura} or an
explicit \texttt{AuthorityBinding}.
