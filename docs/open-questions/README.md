...
- **[discuss-eal-block.md](discuss-eal-block.md)** — whether EAL should grow a `discuss { }` block form mirroring the `easynet discuss` CLI verb. Dropped from the approved `eal-control-flow-v1.md` RFC for lack of a gallery consumer. Trigger: a mission that needs the block form (e.g. conference nested inside a loop's verify, or transcript consumed by a downstream EAL step).
