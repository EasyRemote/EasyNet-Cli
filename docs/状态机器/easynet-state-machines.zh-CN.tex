...
\code{S340} & \statecode{SESSION\_ADMITTED} & CLI daemon / Hub & \code{<self>.session} admission 成功 & No \\
...
\statecode{V1\_SAME\_REALM\_SAME\_USER\_REMOTE\_DEVICE} & 同 realm、同 user、目标 device 在线 & \statecode{FinalRoute(RemoteDeviceAbility)} & hub \code{<self>.invoke\_remote} & \statecode{TARGET\_NOT\_IN\_PRESENCE\_REGISTRY}, \statecode{NOROUTE}, \statecode{STALE} \\
