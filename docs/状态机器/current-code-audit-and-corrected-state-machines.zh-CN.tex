...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Join autostart}: \code{EasyNet-Cli/src/cli/join.rs}. pairing accepted 后默认 start daemon; start 失败显式报错。
  \item \textbf{Runtime start}: \code{EasyNet-Cli/src/cli/start.rs}. credential verify、Hub endpoint probe、daemon boot failure 都记录 snapshot; 仍需保证 online 只在 session admission evidence 后记录。
  \item \textbf{Stop}: \code{EasyNet-Cli/src/cli/stop.rs}. 当前有 \code{StopPlan/StopStage}; 缺少持久化 stop state/code。
  \item \textbf{Reset}: \code{EasyNet-Cli/src/cli/reset.rs}. active runtime 时拒绝 reset; best-effort revoke 后删除本地 credentials; 当前不是 self uninstall。
