...
| Java / JVM SDK | `sdk/java` exposes a P1 Runtime Core seam as a Maven package for typed errors, feature discovery, complete Invocation draft construction, prepared/signed Invocation material over injected runtime transport, `CompletableFuture` async adapters, iterator-backed bounded stream/bidi retained-history state, Health DTO/client seams, Authority metadata projection and injected-transport minting seams, Directory + Identity DTO/client seams including directory subscription carrier/projection/stream-handle lifecycle, Receipt DTO/client seams, Publication carrier/resource-ref seams, Host Binding codec/hash/lifecycle seams, Mission carrier/status/event seams, Admin + Gateway carrier/status/session seams, Events carrier/frame/stream seams, Surface page/manifest/health seams, Compatibility carrier/projection seams, Wrappers record-projection seams, and directly exercised seam action-adapter reports over dependency-free Java classes. | JNI/C ABI or daemon transport adapter, full profile API coverage, full reactive publisher adapters, provider-backed transport evidence, handle terminal observation coverage, and product cutover evidence. |
| Swift SDK | `sdk/swift` exposes a P1 Runtime Core seam as a Swift Package Manager package for typed errors, feature discovery, complete Invocation draft construction, prepared/signed Invocation material over injected runtime transport, `AsyncSequence` stream/bidi retained-history state, Health DTO/client seams, Authority metadata projection and injected-transport minting seams, Directory + Identity DTO/client seams including directory subscription carrier/projection/stream-handle lifecycle, Receipt DTO/client seams, Publication carrier/resource-ref seams, Host Binding codec/hash/lifecycle seams, Mission carrier/status/event seams, Admin + Gateway carrier/status/session seams, Events carrier/frame/stream seams, Surface page/manifest/health seams, Compatibility carrier/projection seams, Wrappers record-projection seams, and directly exercised seam action-adapter reports over dependency-free Swift types. | C ABI or daemon transport adapter, full profile API coverage, provider-backed transport evidence, handle terminal observation coverage, and product cutover evidence. |
...
3. Java and Swift Authority metadata, Mission carrier/status, and Compatibility
   carrier/projection seams pass `authority/mutual_exclusion`,
   `mission/carrier_status`, and `compatibility/openai_carrier_projection`
   before either package is claimed complete for those P1 profile coverage
   areas.
4. No package exposes Axon symbols.
