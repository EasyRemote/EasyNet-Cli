...
| Node / TypeScript SDK | `sdk/node` exposes a P1 seam package for Runtime Core, Health, Authority, Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, and Wrappers with type declarations, Invocation builders, promise-based runtime calls, async iterable stream/bidi handles, bounded stream/bidi backpressure projection, `AbortSignal` cancellation, authority metadata guardrails, and profile DTO/client seams over injected transports. | Daemon/C ABI provider, package stability, and product cutover evidence remain incomplete before Node can claim provider-backed or stable SDK support. |
| Java / JVM SDK | `sdk/java` exposes a P1 Runtime Core seam for typed errors, feature discovery, complete Invocation draft construction, injected runtime transport, and bounded stream/bidi retained-history state over dependency-free Java classes. | JNI/C ABI or daemon transport adapter, package metadata, directory/health/identity/receipt/profile APIs, async/reactive stream idioms, shared action-adapter report, and product cutover evidence. |
| Swift SDK | `sdk/swift` exposes a P1 Runtime Core seam for typed errors, feature discovery, complete Invocation draft construction, injected runtime transport, and bounded stream/bidi retained-history state over dependency-free Swift types. | Swift Package, C ABI or daemon transport adapter, directory/health/identity/receipt/profile APIs, stream/bidi `AsyncSequence`, shared action-adapter report, and product cutover evidence. |
...
| SDK conformance | `sdk/conformance/` contains shared cases, fixtures, fixture-schema bindings, parity matrix, backend route-family coverage data, Rust/C ABI/Go/Python action-adapter reports, a Node seam action-adapter report, Java/Swift seam scaffold guards, and aggregate product smoke gate wiring. | Java/Swift action-adapter reports, per-route live product smoke evidence, and complete profile execution cases remain incomplete. |
