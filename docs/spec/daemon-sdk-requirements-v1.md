...
| Go SDK | `sdk/go` is a provider-backed P0 facade for Runtime Core plus Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, Wrappers, typed errors, backend boundary gates, product smoke gate wiring, and shared conformance runner integration. | Backend route-source cutover, package stability gates, lower-layer deletion proof, and per-route live smoke evidence remain incomplete before backend-cutover-ready or complete SDK support can be claimed. |
| Python SDK | `sdk/python/easynet_sdk` is a provider-backed P0 facade for Runtime Core plus Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, Wrappers, typed errors, EasyRemote boundary gates, and shared conformance runner integration. | EasyRemote repository extraction, non-C ABI live daemon keyring transport, package stability gates, and lower-layer deletion proof remain incomplete before EasyRemote-cutover-ready or complete SDK support can be claimed. |
| Node / TypeScript SDK | `sdk/node` exposes a P1 seam package for Runtime Core, Directory + Identity, and Receipt with type declarations, Invocation builders, promise-based runtime calls, async iterable stream/bidi handles, `AbortSignal` cancellation, DirectoryClient read-model resolve/list/subscribe seams, IdentityClient URA/DescriptorRef projection seams, ReceiptClient fetch/projection/verification/causal-ref/history seams, and shared conformance runner integration over injected transports. | Daemon/C ABI provider, remaining profile clients, package stability, and product cutover evidence remain incomplete before Node can claim provider-backed or stable SDK support. |
...
| SDK conformance | `sdk/conformance/` contains shared cases, fixtures, fixture-schema bindings, parity matrix, backend route-family coverage data, Rust/C ABI/Go/Python action-adapter reports, and aggregate product smoke gate wiring. | Node, Java, Swift, per-route live product smoke evidence, and complete profile execution cases remain incomplete. |
