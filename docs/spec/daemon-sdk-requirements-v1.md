...
| Node / TypeScript SDK | `sdk/node` exposes a P1 seam package for Runtime Core and Directory + Identity with type declarations, Invocation builders, promise-based runtime calls, async iterable stream/bidi handles, `AbortSignal` cancellation, DirectoryClient read-model resolve/list/subscribe seams, and IdentityClient URA/DescriptorRef projection seams over injected transports. | Daemon/C ABI provider, remaining profile clients, conformance action-adapter report, package stability, and product cutover evidence remain incomplete before Node can claim provider-backed or stable SDK support. |
