...
| Node / TypeScript SDK | `sdk/node` exposes a P1 seam package for Runtime Core, Health, Directory + Identity, Receipt, Publication, Host Binding, Mission, Events, Surface, and Compatibility with type declarations, Invocation builders, promise-based runtime calls, async iterable stream/bidi handles, `AbortSignal` cancellation, and profile DTO/client seams over injected transports. | Daemon/C ABI provider, Admin + Gateway and Wrapper profile clients, package stability, and product cutover evidence remain incomplete before Node can claim provider-backed or stable SDK support. |
...
| SDK conformance | `sdk/conformance/` contains shared cases, fixtures, fixture-schema bindings, parity matrix, backend route-family coverage data, Rust/C ABI/Go/Python action-adapter reports, a Node seam action-adapter report, and aggregate product smoke gate wiring. | Java, Swift, per-route live product smoke evidence, and complete profile execution cases remain incomplete. |
