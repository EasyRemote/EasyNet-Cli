...
| --- | --- | --- |
| Rust daemon internals | Daemon process, ability dispatch, invocation routing, C ABI implementation, and daemon Invocation service exist in `src/daemon/` and `src/ffi/`. | A clearly separated public Rust daemon SDK facade with stable DTOs, prepare/sign/submit objects, directory/identity/receipt/publication/host-binding/mission/admin/events/surface/compatibility/health clients, and conformance fixtures. |
| C ABI v3 | `include/easynet_cli.h` exposes ABI version, last error, string free, init/shutdown, daemon start/stop/status/invocation endpoint/open client, unary invoke, stream open/cancel, and bidi open/send/close/cancel. | Feature discovery, explicit attach/discover/detach, complete endpoint DTO, runtime health, invocation builder handles, prepare/sign/submit, invocation handle/events, receipt fetch/verify projection, directory, identity, publication, host binding, mission, admin/gateway, events, surface, compatibility, typed error JSON, and schema-backed terminal stream/bidi events. |
| Go SDK | `sdk/go/README.md` is a placeholder facade marker. | Package implementation, public module path, lifecycle/runtime/invocation/directory/health/identity/receipt/profile APIs, Hub events/surface/compatibility/wrapper APIs, typed errors, backend import-ban tests, profile conformance, and conformance runner. |
| Python SDK | `sdk/python/README.md` is a placeholder facade marker. | Package implementation or C ABI bridge, EasyRemote extraction boundary, complete Invocation codec, signing material transport, receipt fetch/verify projection, directory/identity, publication profile, host binding profile, mission profile, admin/gateway profile, events/surface/compatibility profiles where shipped, typed errors, async stream/bidi adapters, and conformance runner. |
| Node / TypeScript SDK | `sdk/node/README.md` is a placeholder facade marker. | Package implementation, type declarations, async iterable stream/bidi APIs, profile clients for shipped profiles, ABI/N-API or daemon transport strategy, and conformance runner. |
| Java / JVM SDK | `sdk/java/README.md` is a placeholder facade marker. | Java API, JNI/C ABI or daemon transport adapter, lifecycle/runtime/invocation/directory/health/identity/receipt/profile APIs, stream/bidi idioms, and conformance runner. |
| Swift SDK | `sdk/swift/README.md` is a placeholder facade marker. | Swift Package, C ABI or daemon transport adapter, async/await lifecycle/runtime/invocation/directory/health/identity/receipt/profile APIs, stream/bidi `AsyncSequence`, and conformance runner. |
| SDK schemas | No `sdk/schemas/` directory is present in the current tree. | Invocation, receipt, error, health, event, directory page, stream event, bidi frame, authority, lifecycle status, publication, resource ref, host-stream binding/frame, mission, admin, gateway, agent-record, surface/page, compatibility, file, terminal, remote-desktop, browser-session, and media-session schemas. |
| SDK conformance | No `sdk/conformance/` directory is present in the current tree. | Golden cases and runner for Rust, C ABI, Go, Python, Node, Java, and Swift. |
| Interface docs | This requirements document exists; `docs/spec/ffi-abi-v3.md` documents the current Invocation-only ABI. | `sdk/SDK_INTERFACE_SPEC.md`, `sdk/SDK_PARITY.md`, and `sdk/CONFORMANCE_SUITE.md` still need to be added as implementation-facing contracts. |
