...
| Java / JVM SDK | `sdk/java` exposes a P1 Runtime Core seam for typed errors, feature discovery, complete Invocation draft construction, injected runtime transport, bounded stream/bidi retained-history state, and directly exercised seam action-adapter reports over dependency-free Java classes. | JNI/C ABI or daemon transport adapter, package metadata, directory/health/identity/receipt/profile APIs, async/reactive stream idioms, provider-backed transport evidence, and product cutover evidence. |
