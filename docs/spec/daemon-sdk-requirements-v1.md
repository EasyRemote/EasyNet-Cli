...
| SDK conformance | `sdk/conformance/` contains shared cases, fixtures, fixture-schema bindings, parity matrix, backend route-family coverage data, Rust/C ABI/Go/Python action-adapter reports, a Node seam action-adapter report, Java seam scaffold guard, and aggregate product smoke gate wiring. | Java action-adapter report, Swift, per-route live product smoke evidence, and complete profile execution cases remain incomplete. |
