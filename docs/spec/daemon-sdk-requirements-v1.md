...
| Go SDK | `sdk/go` is a provider-backed P0 facade for Runtime Core plus Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, Wrappers, typed errors, backend boundary gates, and shared conformance runner integration. | Backend route-source cutover, external product smokes, package stability gates, and lower-layer deletion proof remain incomplete before backend-cutover-ready or complete SDK support can be claimed. |
...
| SDK conformance | `sdk/conformance/` contains shared cases, fixtures, fixture-schema bindings, parity matrix, backend route-family coverage data, and Rust/C ABI/Go/Python action-adapter reports. | Node, Java, Swift, external product cutover smokes, and complete profile execution cases remain incomplete. |
