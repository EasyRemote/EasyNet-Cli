...
| Swift SDK | `sdk/swift` exposes a P1 Runtime Core seam as a Swift Package Manager package for typed errors, feature discovery, complete Invocation draft construction, injected runtime transport, `AsyncSequence` stream/bidi retained-history state, and directly exercised seam action-adapter reports over dependency-free Swift types. | C ABI or daemon transport adapter, directory/health/identity/receipt/profile APIs, provider-backed transport evidence, and product cutover evidence. |
