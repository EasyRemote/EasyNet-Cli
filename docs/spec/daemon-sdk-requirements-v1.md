...
| Swift SDK | `sdk/swift` exposes a P1 Runtime Core seam as a Swift Package Manager package for typed errors, feature discovery, complete Invocation draft construction, injected runtime transport, and bounded stream/bidi retained-history state over dependency-free Swift types. | C ABI or daemon transport adapter, directory/health/identity/receipt/profile APIs, stream/bidi `AsyncSequence`, shared action-adapter report, and product cutover evidence. |
