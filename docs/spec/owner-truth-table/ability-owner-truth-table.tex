...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  resource. Companion to \codew{voice.subscribe} (TTS).
\end{description}
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  \item[\codew{policy.evaluate}, \codew{policy.simulate}]
  RFC §A6 admission policy. The kernel admission gate calls
  \codew{policy.evaluate} as an in-process sub-invocation
  carrying the original envelope; \codew{policy.simulate} is
