...
\begin{longtable}{@{}p{0.35cm} L{4.0cm} L{3.9cm} L{3.6cm} L{3.4cm}@{}}
  ...
  3 & \codew{device.fleet.*}, \codew{device.observe.*}, \codew{device.admin.*}, \codew{device.discuss.*}, \codew{device.loop.*}, \codew{device.schedule.*}, \codew{device.mission.*}, \codew{device.meta.*}, \codew{device.ability.*}, \codew{device.policy.*}, \codew{device.consent.*}, \codew{device.skill.*}, \codew{device.mcp.*}, \codew{device.a2a.*}, \codew{device.voice.*} &
      \code{device/<id>} &
      \code{=callee} (meta) &
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  hub unreachable".
  \item[\codew{device.mission.track}, \codew{device.mission.cancel}]
  Mission lifecycle introspection + termination. \code{track}
  reads the persisted run dir of a prior \codew{device.mission.run}
