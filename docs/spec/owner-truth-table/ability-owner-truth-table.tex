...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.1em,topsep=0.1em]
  \item \emph{What URA names its owner} (the \code{owner\_agent\_uri}
...
(\codew{src/uri.rs::parse\_ura}) rejects any string outside the six
shapes below. Other "URA-shaped" tokens that have appeared in practice
(\codew{<self>}, \codew{<unjoined>}, \code{agent://self}) are
...
\begin{tabularx}{\linewidth}{l l X}
  ...
  \code{hub}      & \codew{easynet:///r/<realm>/hub}                                        & realm-singleton; no tail; no sub-id (the legacy \code{/hub/01HUB} tail is retired) \\
...
\begin{description}[leftmargin=1.4em,style=nextline]
  ...
  string that requires caller context to resolve (\codew{<self>}, the
  ...
  from the name] The descriptor's \code{owner\_agent\_uri} is the
...
\begin{tabularx}{\linewidth}{l l X}
  ...
  \code{function\_name} & string, not a URA & The ability name (\codew{device.fs.read}, \codew{<owner>.chat}, \codew{hub.openai.chat_completions}, \dots). \\
  \bottomrule
\end{tabularx}
...
\code{owner\_agent\_uri}, the resulting \code{envelope.callee}, and
...
\begin{longtable}{@{}p{0.35cm} L{4.0cm} L{3.9cm} L{3.6cm} L{3.4cm}@{}}
  ...
  3 & \codew{device.fleet.*}, \codew{device.observe.*}, \codew{device.admin.*}, \codew{device.discuss.*}, \codew{device.loop.*}, \codew{device.schedule.*}, \codew{device.mission.*}, \codew{device.easynet.*}, \codew{device.meta.*}, \codew{device.ability.*}, \codew{device.policy.*}, \codew{device.consent.*}, \codew{device.skill.*}, \codew{device.mcp.*}, \codew{device.a2a.*}, \codew{device.voice.*} &
      \code{device/<id>} &
      \code{=callee} (meta) &
  ...
  5 & \codew{device.keyring.*}, \codew{device.session}, \codew{device.register_device_pubkey} &
      \code{device/<id>} &
  ...
  8 & \code{consent.*}, \code{policy.*} (handler form), \code{mcp.bridge.*}, \code{mcp.client.*}, \code{a2a.bridge.*}, \code{a2a.client.*}, \code{voice.*} (handler form), \code{skill.*} &
      ...
      \codew{profiles::consent}; \codew{profiles::policy}; \codew{profiles::mcp}; \code{voice} module \\

  9 & \codew{hub.openai.*}, \codew{hub.<namespace>.*} &
      \code{hub} (\codew{easynet:///r/<r>/hub}, realm-singleton) &
      \code{=callee} or per-verb (e.g.\ \codew{chat_completions} carries no subject) &
      \codew{openai_compat_ability::register} \\
...
\begin{description}[leftmargin=1.4em,style=nextline]
  ...
  with \code{owner\_agent\_uri = device URA}. They are not per-agent.
  ...
  \codew{hub_uri(credentials.tenant_id)} --- not from the device URA.
...
\code{owner\_agent\_uri} into the columns
...
on a freshly-paired daemon (realm \codew{device.easynet.run}, user \code{test},
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.fs.edit}, \codew{device.fs.list}, \codew{device.fs.read}, \codew{device.fs.write}]
  Filesystem locomotion. \codew{device.fs.edit} is the surgical
  string-replace primitive that lets agents diff-apply rather than
  full-rewrite (audit-trail compactness).
  \item[\codew{device.http.request}] One outbound HTTP request per
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.camera.snapshot}, \codew{device.camera.subscribe}] Single
  still vs.\ pushed video stream from a camera resource.
  \item[\codew{device.mic.subscribe}] Pushed audio stream from a microphone
  resource.
  \item[\codew{device.screen.snapshot}, \codew{device.screen.subscribe}] Single
  still vs.\ pushed video stream of a display target.
  \item[\codew{device.speaker.publish}] Outbound audio frames to a speaker
  resource. Companion to \codew{device.voice.subscribe} (TTS).
\end{description}
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  \codew{device.fleet.register_self}, \codew{device.fleet.deregister_self},
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.admin.status}] Operator-facing component snapshot
  ...
  \item[\codew{device.observe.health}] Round-trip smoke ability. Returns
  the caller's args plus a daemon timestamp. The minimal proof of
  liveness; every CI sanity check uses this.
  \item[\codew{device.observe.network_health}] Live network posture: local
  ...
  hub unreachable".
  \item[\codew{device.easynet.discover}, \codew{device.easynet.run},
  \codew{device.easynet.cancel}, \codew{device.easynet.track}]
  User-facing aliases that LLM prompts read more naturally than
  the literal \code{meta.list\_abilities} / \codew{device.mission.run} /
  \codew{device.loop.cancel} / \codew{device.loop.status}. Both names route to the
  same handler so the catalogue and dispatch table stay in lockstep.
\end{description}

\subsection{Device-owned --- mission, scheduling, loops, discuss (under \texttt{\small device.*})}

\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.mission.run}, \codew{device.mission.think},
  \codew{device.mission.discuss_round}]
  ...
  \codew{device.mission.discuss_round} drives one round of multi-agent
  conversation; the underlying state shares the
  \code{discuss.*} room model.

  \item[\codew{device.schedule.add}, \codew{device.schedule.list},
  \codew{device.schedule.enable}, \codew{device.schedule.remove}]
  ...
  \item[\codew{device.loop.create}, \codew{device.loop.status},
  \codew{device.loop.subscribe}, \codew{device.loop.cancel}]
  ...
  \item[\codew{device.discuss.create}, \codew{device.discuss.post},
  \codew{device.discuss.list_turns}, \codew{device.discuss.subscribe}]
  ...
  \codew{device.mission.discuss_round} writes into this same store.
\end{description}

\subsection{Device-owned --- edge adapters and policy (under \texttt{\small device.*})}

\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.a2a.bridge.list_skills}, \codew{device.a2a.bridge.send_task}]
  ...
  \item[\codew{device.a2a.client.send_task}] Outbound A2A: dispatch an
  RPC against a remote A2A agent. The other half of the
  agent-to-agent protocol bridge.

  \item[\codew{device.mcp.bridge.list_tools}, \codew{device.mcp.bridge.call_tool}]
  ...
  \item[\codew{device.mcp.client.call}, \codew{device.mcp.client.list}]
  Outbound MCP: dial a remote MCP server, list and call its
  tools. Pairs with the bridge.

  \item[\codew{device.policy.evaluate}, \codew{device.policy.simulate}]
  RFC §A6 admission policy. The kernel admission gate calls
  \codew{device.policy.evaluate} as an in-process sub-invocation
  carrying the original envelope; \codew{device.policy.simulate} is
  the operator-facing dry-run. Without these the daemon has no
  policy hooks at all and every Invoke is admitted unchecked.
\end{description}

\subsection{Device-owned --- ability publishing primitives (under \texttt{\small device.ability.*})}

\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.ability.publish}, \codew{device.ability.unpublish}]
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.keyring.create}, \codew{device.keyring.list},
        \codew{device.keyring.get_public}, \codew{device.keyring.sign},
  ...
  \item[\codew{device.session}] The long-lived bidi RPC session
  ...
  \item[\codew{device.register_device_pubkey}]
...
% ---------- Sub-agent voice / meta / skill (16 rows) -------------------
\subsection{Agent-owned (LLM sub-agent surfaces): voice, meta, skill}

These verbs live under \code{web-builder}'s sub-agent in this
fixture --- they are re-exported per agent, not device-wide.

\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.meta.describe}, \codew{device.meta.list_abilities},
        \codew{device.meta.list_resources}]
  ...
  \item[\codew{device.skill.list}, \codew{device.skill.publish}, \codew{device.skill.unpublish}]
  ...
  \item[\codew{device.voice.create_call}, \codew{device.voice.join_call},
        \codew{device.voice.leave_call}, \codew{device.voice.end_call},
        \codew{device.voice.list_calls}, \codew{device.voice.show_call},
        \codew{device.voice.watch_call}, \codew{device.voice.report_metrics}]
  Voice/video call signaling. SDP, ICE, participant book-keeping.
  Without the signaling surface there is no protocol path for an
  agent to start or join a call --- WebRTC needs an out-of-band
  rendezvous and these abilities provide it through Invoke.

  \item[\codew{device.voice.subscribe}, \codew{device.voice.transcribe}]
  Media planes: \code{subscribe} pushes TTS audio frames out;
  \code{transcribe} accepts streamed audio in and returns text.
  Together they make the agent voice-bidirectional.
\end{description}
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  \item[\codew{device.consent.subscribe}] Subscribe to the
  approval-broker pending queue. \code{SnapshotThenLive} stream;
  a UI joining mid-flight gets currently-pending requests + new
  ones.
  \item[\codew{device.consent.list_pending}] One-shot snapshot of the
  same queue (the unary sibling).
  \item[\codew{device.consent.decide}] Resolve a pending permission
...
\begin{itemize}[leftmargin=1.6em,itemsep=0.1em]
  ...
  \item \codew{src/uri.rs} --- canonical URA builders + parser; the §1 grammar is reflected here verbatim.
  \item \codew{src/runtime/ability_descriptor.rs} --- \code{AbilityDescriptor.owner\_agent\_uri}; v4.1.5 §9 \code{callee} constraint comment.
  \item \codew{src/runtime/agents/meta_ability.rs} --- \code{list\_abilities\_handler} synth path.
  \item \codew{src/facade/cli/abilities.rs} --- \code{easynet ability list} render layer.
\end{itemize}
