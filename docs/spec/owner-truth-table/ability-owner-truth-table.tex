...
\begin{tabularx}{\linewidth}{l l X}
  ...
  \code{hub}      & \codew{easynet:///r/<realm>/authority}                                  & realm-singleton; no tail; no sub-id; extra tail segments are invalid \\
...
by a device-sponsored SystemAgent selected by ability family. The Device URA
remains the execution host, resource host, custody anchor, or subject when the
invocation addresses physical/local state; it is not the ordinary public
callee for these descriptors.
...
\begin{longtable}{@{}p{0.35cm} L{4.0cm} L{3.9cm} L{3.6cm} L{3.4cm}@{}}
  ...
  3 & \codew{device.fleet.*}, \codew{device.observe.*}, \codew{device.admin.*}, \codew{device.discuss.*}, \codew{device.loop.*}, \codew{device.schedule.*}, \codew{device.mission.*}, \codew{device.meta.*}, \codew{device.ability.*}, \codew{device.policy.*}, \codew{device.consent.*}, \codew{device.skill.*}, \codew{device.mcp.*}, \codew{device.a2a.*} &
      device-sponsored SystemAgent for the family\newline
      (e.g.\ \code{agent/device.<id>.node-management}, \code{agent/device.<id>.runtime-*}, \code{agent/device.<id>.automation}, \code{agent/device.<id>.ability-management}, \code{agent/device.<id>.skill-management}, \code{agent/device.<id>.plugin-management}, \code{agent/device.<id>.a2a-integration}) &
      \code{=callee} (meta) &
      \codew{profiles::*}; runtime \codew{register_rpc} with explicit SystemAgent owner \\
  ...
  5 & \codew{device.keyring.*}, \codew{session.open}, \codew{identity.register_pubkey} &
      \code{agent/device.<id>.keyring-management} for \codew{device.keyring.*};\newline
      session/identity bootstrap authority per transport path &
      \code{=callee} (RFC-002 §3.3) &
      \codew{keyring::abilities}\newline
  ...
  8 & \code{consent.*}, \code{policy.*} (handler form), \code{mcp.bridge.*}, \code{mcp.client.*}, \code{a2a.bridge.*}, \code{a2a.client.*}, \code{skill.*} &
      \code{agent/<u>.<helper-id>} for hosted helper agents (e.g.\ \codew{consent-default-0}); device-sponsored integration/SystemAgent for daemon-side adapters &
  ...
  9 & \code{voice.*}, \codew{hub.openai.*}, \codew{hub.<namespace>.*} &
      \code{hub} (\codew{easynet:///r/<r>/authority}, realm-singleton) &
  ...
  10 & \code{<user>.pages.*}, \codew{<user>.<project>.page.fetch}, \code{<user>.files.*} &
      \code{agent/<u>.<owner-tail>} for callable surfaces;\newline
      \code{user/<u>} only as principal/resource-accountability owner &
  ...
  11 & \codew{<user>.api_key.*}\newline
      ...
      \code{agent/<u>.api-key-management} or explicit owning Agent;\newline
      \code{user/<user>} is principal/accountability, not a callable Agent &
      \code{=callee} (meta) &
      \codew{api_key_ability::register} \\
...
\begin{description}[leftmargin=1.4em,style=nextline]
  ...
  \item[Row 5 is daemon-local but not direct Device ownership] Daemon-bundle abilities
  (\code{device.keyring.*}, \code{session.open},
  \code{identity.register\_pubkey}) are mounted once per daemon. Keyring
  descriptors publish through the device-sponsored \code{keyring-management}
  SystemAgent. Session and identity bootstrap routes are transport/admission
  primitives with explicit bootstrap authority; they must not be generalized
  into ordinary direct-Device descriptor ownership. They are not per-user-agent.
  ...
  (\code{mcp.bridge.list\_tools}, owner = a device-sponsored integration
  SystemAgent such as \code{a2a-integration} or plugin/MCP-specific agent).
  The catalogue records the sub-case at registration time --- see~P2.
  ...
  defaults are not part of the spec. A User account is a principal and
  accountability subject, not an Agent and not an ability-advertising callee.
\end{description}
...
\begin{tabularx}{\linewidth}{l l l l X}
  ...
  \midrule
  \code{device/<id>}            & \code{<id>} & \code{-}    & \code{-}    & Device-as-subject/host or migration-only legacy rows; not rows~1, 2, 3, 5 descriptor ownership \\
  \code{agent/device.<id>.<system-agent>} & \code{<id>} & \code{<system-agent>} & \code{-} & rows~1, 2, 3, 5, 8 daemon-side adapters \\
  \code{agent/<u>.<a>}          & \code{-}    & \code{<a>}  & \code{<u>}  & rows~4, 6, 7, 8a, 10 callable user-hosted agent surfaces \\
  \code{hub}                    & \code{-}    & \code{hub}  & \code{-}    & row~9 \\
  \code{user/<u>}               & \code{-}    & \code{-}    & \code{<u>}  & principal/resource-accountability projection only; not an ability callee \\
...
\subsection{SystemAgent-owned, Device-hosted --- AXIOM Tier 2.5 baseline-locomotion-v1 (under \texttt{\small device.*})}
...
contract.
The public names remain \codew{device.*}, but descriptor ownership is the
device-sponsored \code{locomotion} or \code{terminal} SystemAgent, with the
Device acting as execution host and local custody anchor.
...
\subsection{SystemAgent-owned, Device-hosted --- media resources (under \texttt{\small device.*}, subject required)}
...
The media SystemAgent owns the descriptors; the Device hosts the physical
handles named by the subject.
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  output into this Device-hosted media sink through a separate Invocation.
\end{description}

\subsection{SystemAgent-owned, Device-hosted --- fleet operational surface (under \texttt{\small device.fleet.*})}
...
Per RFC §18 the Device selects the target host. Descriptor owner/callee remains
the family SystemAgent; dynamic ability deployment in particular publishes
through \code{ability-management}, not the Device URA.
...
\subsection{SystemAgent-owned, Device-hosted --- observability, admin, discovery aliases (under \texttt{\small device.*})}
...
\subsection{SystemAgent-owned, Device-hosted --- mission, scheduling, loops, discuss (under \texttt{\small device.*})}
...
\subsection{SystemAgent-owned, Device-hosted --- edge adapters and policy (under \texttt{\small device.*})}
...
\subsection{SystemAgent-owned, Device-hosted --- ability publishing primitives (under \texttt{\small device.ability.*})}
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  Runtime deployment/uninstall rows are owned by the device-sponsored
  \code{ability-management} SystemAgent. The target Device is the execution
  host and deployment subject, not the descriptor owner.
\end{description}

\subsection{SystemAgent-owned / bootstrap authority --- daemon device-bundle (row 5)}

These are RFC-002 §3.3 abilities the daemon hosts directly, with
no user-agent wrapper. The catalogue carries exactly one row each. Keyring
descriptors are owned by \code{keyring-management}; \code{session.open} and
\code{identity.register_pubkey} are bootstrap transport/admission primitives,
not evidence that the Device is an ordinary ability owner.
...
Microphone and speaker handles remain Device-hosted resources under
\codew{mic.*} and \codew{speaker.*}.  The \code{voice.*} namespace is
realm-shared state and therefore has one canonical owner:
\codew{easynet:///r/<r>/authority}.  The call registry is keyed by the pair
...
\begin{description}[leftmargin=1.4em,style=nextline,itemsep=0.15em]
  ...
  Device-hosted microphone and speaker Invocations.
\end{description}
