...
4. `src/daemon/control/` for local boot/status IPC, `src/daemon/invocation/`
   for daemon-owned Invocation transport/admission/session dispatch, and
   `src/services/` for the remaining migration-stage trust, federation, quota,
...
5. `invocation/` owns admission, routing, dispatch, receipts, streams, and bidi
   session semantics. It does not own product handler bodies.
...
8. Existing public Ability names remain byte-identical.
9. `meta.list_abilities` returns the same ability names before and after a
   structural move.
10. Ability call modes remain unchanged.
11. Descriptor generation output remains byte-identical unless the phase is
    explicitly a descriptor-format change.
12. No product-module source move changes Invocation or Receipt semantics.
13. No runtime registry tree is introduced.

Complexity/fan-out:

14. Ordinary list methods satisfy their documented complexity contracts.
15. Ordinary list/facade methods do not run per-agent/per-device governed
    fan-out loops.
16. Any aggregate fan-out is a named daemon/hub aggregate ability with bounded
    concurrency, deadline, partial result semantics, child receipt refs, and
    typed per-target errors.
17. Facade methods that invoke aggregate abilities are named as aggregate
    helpers.
...
21. Descriptor lookup uses root, per-ability path, or iterator helpers and does
    not assume a flat descriptor directory.
22. Skills are not treated as protocol-callable objects unless wrapped by an
    explicit AbilityDescriptor.
23. Axon protocol semantics are not duplicated in CLI/backend/facade code.
