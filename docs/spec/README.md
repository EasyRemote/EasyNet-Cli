...
- **[canonical-runtime-convergence-v2.md](canonical-runtime-convergence-v2.md)** — Cross-repository clean-target SPEC for Axon/EasyNet-Cli convergence: product-neutral SDK ownership, one descriptor-bound invocation/proof path, explicit signer custody, language lifecycle parity, LocalRuntime-only daemon invocation, Mission/EAL extraction, URA terminology, and executable acceptance gates.
- **[canonical-runtime-hardening-and-redundancy-retirement-v1.md](canonical-runtime-hardening-and-redundancy-retirement-v1.md)** — Follow-up convergence hardening SPEC for retiring duplicated descriptor resolution authority, cross-language signer policy drift, receipt proof-fact bypass risk, typed route/admission instability, and missing product mutation evidence after the primary V2 gate is green.
