...
\texttt{principal/} caller URAs, a content-as-blob boundary, and a
...
\texttt{owner\_agent} URA inside the state key is the standard
...
field schema is delegated to the call site (\S\ref{sec:caller-ura}
specifies one in the principal-URA context). RFC-006 main commits
...
by an EasyNet Agent URA. The owner agent is the \emph{canonical
...
\subsection{The principal/ caller URA scheme}
\label{sec:caller-ura}

\normsv{}
The \texttt{envelope.caller} field of every invocation \textbf{MUST}
be a URA under one of two namespaces:
...
Acceptance of \texttt{principal/} URAs requires extending the parser,
...
TR-INV-12 & Hub-translated caller identity                     & \S\ref{sec:caller-ura} \\
...
\begin{enumerate}[leftmargin=2.5em,itemsep=0.3em]
    ...
    \item \textbf{1c. \texttt{principal/} caller URA parser and
