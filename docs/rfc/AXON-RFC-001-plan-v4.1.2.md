...
inlined the §19 mapping table but left two foundational gaps and a
handful of clarifications open. v4.1.2 closes those, becoming the
binding contract.

The 9 patches inherited from v4.1.1:
...
The 8 additional patches landing in v4.1.2:

| # | Topic | Section |
|---|---|---|
| R1 | Operator URA provisioning lifecycle | §1.7 (new), §1 (human entry), §3 Step D, §18 (identity.*), §19 |
| R2 | Local-host issuer exception for device-profile DelegationProofs | §1.5 rule 2.5 (new), §3 Step C/D, §16, §A14, §A16 (new) |
| M1 | Receipt schema explicit for self-signing Agents | §1.3 |
| M2 | Audience matcher precision (exact / prefix-with-boundary / `*`) | §1.5 |
| M3 | ScopeRule replaces empty-list ambiguity (`Any` vs `OnlyMatching`) | §1.6, §18 (PRIVATE / SCOPED entries) |
| M4 | local-agents.json hub-side recovery before re-mint | §1.4 |
| M5 | RealmDirectory cache TTL default + override path | §1.2, §16, §A11 |
| M6 | `easynet runtime stop` ordering + §16 cross-tenant rule | §16, §19 |
...
human  (operator)
  ...
  uri shape:         easynet:///r/acme/agent/01USR-<ulid>
                     (URA exists for audit; no daemon hosts it)
  provisioning:      see §1.7 — minted by hub-profile on first
                     observation; backend-profile keeps the
                     (auth_provider, external_subject) → URA cache
```
...
**Cache TTL (M5).** Each cached `DirectoryEntry` carries an `expires_at`
timestamp. Default TTL is `max(heartbeat_ms × 6, 60s)` from the time of
the last `federation.resolve` or `realm_directory_delta` that produced
the entry. Hub MAY push fresher TTLs via `realm_directory_delta` (the
delta payload includes `default_entry_ttl_ms`). Daemon config
`runtime.directory_cache_ttl_ms` overrides locally for testing. An
expired entry is treated as "cache miss" in step 2 above.
...
**Receipt schema is uniform across both models (M1).** Every Receipt
carries `callee_agent_uri`, `signer_agent_uri`, and an OPTIONAL
`host_attestation`. For Model A:

- `signer_agent_uri == callee_agent_uri`
- `host_attestation` is absent (`null`)
- Verifier short-circuits: validate the receipt signature against the
  callee's own public key in RealmDirectory; do not look up host
  attestation.

For Model B: the three fields are populated as described above. A
receipt without `host_attestation` whose `signer_agent_uri` is
`hosted_by:`-attested in RealmDirectory MUST be rejected — the schema
mismatch indicates a forged "self-signed" receipt for a hosted Agent.
...
4. **Recovery before re-mint (M4).** On daemon boot, if
   local-agents.json is missing but credentials.json exists, the
   daemon SHOULD attempt hub-side recovery before minting fresh URIs:
   - Issue `federation.resolve { filter: { hosted_by:
     <this-device's-canonical-URA> } }`.
   - For each returned hosted Agent whose `(profile, name)` matches a
     currently-enabled local profile and whose host_attestation in
     RealmDirectory verifies against this device-profile's key, adopt
     the existing canonical URA into local-agents.json.
   - For profiles with no recoverable entry, mint fresh URIs.
   This avoids gratuitous URA churn after losing only the cache while
   credentials remain intact.
5. If recovery fails (hub unreachable AND no local-agents.json), the
   device-profile re-mints fresh URIs and advertises them as new
   Agents; hub MAY garbage-collect stale entries via TTL on heartbeat
   absence. Operator MAY re-issue `federation.revoke` for orphaned
   prior URAs.
...
DelegationProof {
  ...
  audience            // AudiencePattern (see M2 below)
...
**AudiencePattern (M2).** The audience field MUST match exactly one of:
- `Exact(uri)` — envelope.callee must equal `uri` byte-for-byte;
- `Prefix(uri_prefix)` — envelope.callee must begin with `uri_prefix`
  AND the next character (or end-of-string) MUST be `/`. This
  prevents `easynet:///r/acme/agent/01DEV` from accidentally
  matching `01DEVIL`. Implementations MUST reject prefixes that do
  not end at a path-segment boundary;
- `Any` — no restriction (must be explicit; cannot be omitted).
...
2.5. **Local-host issuer exception (R2).** A device-profile Agent is
   implicitly trusted as DelegationProof issuer — without appearing in
   the hub's trusted-issuer list — iff ALL of the following hold:
   - `subject_uri` equals the operator URA recorded in
     `RealmDirectory[<this-device>].metadata.host_owner_operator_uri`
     (set during `federation.join`; see §3 Step C/D);
   - `audience` is one of: (a) `Exact(<this-device>'s URA)`, (b)
     `Prefix(<this-device>'s URA + "/")` — i.e. any Agent hosted by
     this device, or (c) `Exact(<hub-profile URA>)` — so the operator
     may browse the realm catalogue through their own daemon.
   - `expires_at - issued_at <= 5 minutes` — short-lived only.
   ...
   socket already authenticates "this is the human who owns this
   host." It does NOT widen authority beyond the local host.
3. **Caller match**: caller_uri MUST equal envelope.caller. Otherwise
   reject — caller cannot reuse another Agent's delegation.
4. **Audience match**: envelope.callee MUST satisfy AudiencePattern
   per the rules above.
...
DELEGATION_INVALID, reason }`. When the envelope is delegation-free
(caller == subject), the gate skips rules 1–6 and verifies only the
caller signature (base layer); subject defaults to caller for audit.
