...
%     INV-2  Capability-URA Key — API keys are minted by keyring
...
\lstdefinestyle{ealstyle}{
    ...
    morekeywords={ability,resource,user,agent,hub,kind,realm,owner,ura,subject,caller,callee,receipt,call,let,mission,with,timeout,on,fn,struct,impl,pub,async,await,assume,axiom,invariant,capability},
...
\title{
    ...
    \textbf{\Large Capability-URA API Keys}\\[0.4em]
...
have mutated through the regular dispatch path), Capability-URA
...
\subsection{INV-2 --- Capability-URA Key}
...
\begin{enumerate}[leftmargin=2em,itemsep=0.1em]
    ...
    \begin{itemize}[leftmargin=1.5em,itemsep=0.05em]
        \item \texttt{ability\_ura = ability/01HUB.openai.chat\_completions}
        \item \texttt{caller\_ura = hub/01HUB}
        \item \texttt{callee\_ura = hub/01HUB}
        \item \texttt{subject\_ura = resource/api\_key.<id>}
...
\begin{lstlisting}
caller   = caller_agent_ura
callee   = caller_agent_ura                   (self-call)
...
(the Hub did not invent the reply), Capability-URA Key (the
