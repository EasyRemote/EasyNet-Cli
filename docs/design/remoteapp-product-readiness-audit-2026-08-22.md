...
token. This still is not full crash/restart product recovery: media/input
transports are intentionally degraded after process restart, and the live
verifier still needs evidence for media reattachment, rendered frames, endpoint
cleanup, and cross-process restart behavior.
