...
binding for session abilities, connected WebRTC state, attached media stream
...
requires connected WebRTC selected candidate-pair evidence, a matching
`selected_route_class`, rendered media, public RemoteApp session abilities,
selected Resource URA subject binding, credential redaction, and visible
