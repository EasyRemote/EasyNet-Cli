...
The native ScreenCaptureKit + VideoToolbox WebRTC media loop now emits
...
pipeline id, negotiated H.264 payload content type, WebRTC transport,
requested/effective/measured FPS, target/observed bitrate, bounded stale-frame
drop policy, and drop/backpressure/adaptation events. It still reports
`host_audio_not_implemented` and does not replace the live media adaptation E2E
artifact.
