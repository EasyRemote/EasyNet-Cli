...
It is the admission whitelist for caller URIs and their Ed25519 public keys.

## File format

```toml
[[trusted_agent]]
agent_uri = "easynet:///r/acme/agent/backend"
public_key_b64 = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
role = "backend"
added_at_unix_ms = 1714492800000

[[trusted_agent]]
agent_uri = "easynet:///r/acme/agent/device-01"
...
- `agent_uri` is the canonical URA key. It must be unique within the file.
...
- Duplicate `agent_uri` entries are rejected at load time.
...
1. validates the target URI belongs to the daemon's realm,
...
If the edit also accompanies daemon-config or TLS-certificate changes, use a full
`systemctl restart easynet-daemon` instead.

## Production expectation

- `backend` should appear in the file before the production canary.
- Every paired device should have one stable `agent_uri` entry.
