...
SCRIPT="$REPO_ROOT/scripts/check-pages-api-ability-ura.sh"
...
make_sandbox() {
    local sandbox
    sandbox="$(mktemp -d)"
    mkdir -p "$sandbox/src/runtime/agents/pages" "$sandbox/docs"
    cp "$REPO_ROOT/src/runtime/agents/pages/api.rs" "$sandbox/src/runtime/agents/pages/api.rs"
...
SB="$(make_sandbox)"
perl -0pi -e 's/ability_ura: Option<String>/ability: Option<String>/' "$SB/src/runtime/agents/pages/api.rs"
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
rm -rf "$SB"
...
SB="$(make_sandbox)"
perl -0pi -e 's/AbilitySelector::parse\(&ability_ura\)/crate::ura::parse_ura(&ability_ura)/' "$SB/src/runtime/agents/pages/api.rs"
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
rm -rf "$SB"
