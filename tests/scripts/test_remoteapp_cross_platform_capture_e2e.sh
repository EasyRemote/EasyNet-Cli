...
if "$SCRIPT" --run --evidence-json "$OUT_DIR/good/evidence.json" \
    --out-dir "$OUT_DIR/self-test-as-live" >/tmp/remoteapp-cross-platform-capture-origin.out 2>&1; then
  fail "verifier accepted contract self-test evidence in run mode"
fi
grep -q "evidence_origin must be live_runner" /tmp/remoteapp-cross-platform-capture-origin.out || \
