...
# overwrite + the `axon-runtime` cleanup that production packaging/release/install.sh
# does, so the system can drift between dev runs.
#
# This script is the dev-loop counterpart to `packaging/release/install.sh`. It does the
...
#
# Usage:
#   packaging/release/dev-install-local.sh              # release build, then install
#   packaging/release/dev-install-local.sh --debug      # debug build, faster iter
#   packaging/release/dev-install-local.sh --no-install # build only, print path
#
...
# packaging/release/install.sh:43-50). The dendrite bridge .dylib goes under the invoking
...
# Mirror packaging/release/install.sh's stale-binary cleanup (lines 264-276): if a
