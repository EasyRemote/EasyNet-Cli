...
| Java / JVM facade | Runtime Core seam as a Maven package for typed errors, feature discovery, complete Invocation drafts, injected runtime transport, `CompletableFuture` async adapters, iterator-backed bounded stream/bidi state, and Health DTO/client seams; daemon providers remain unsupported |
| Swift facade | Runtime Core seam as a Swift Package Manager package for typed errors, feature discovery, complete Invocation drafts, injected runtime transport, `AsyncSequence` bounded stream/bidi state, and Health DTO/client seams; daemon providers remain unsupported |
