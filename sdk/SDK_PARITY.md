...
| Go | P0 | EasyNet backend/Hub | Runtime Core discovery/connection/health/errors/invocation-draft/unary/handle/prepare-submit seam partial |
| Python | P0 | EasyRemote | Runtime Core discovery/connection/health/errors/invocation-draft/unary/handle/prepare-submit seam partial |
...
- Go package exposes Runtime Core feature/version discovery, runtime connection
  ...
  DTOs, unary InvocationResult projection, InvocationHandle await/cancel/events
  observation, and RuntimeClient invoke/prepare/submit-signed methods behind
  narrow JSON transport seams; concrete daemon lifecycle/transport, stream/bidi
  adapters, profile clients, backend import-ban integration, per-profile error
  source refs, and conformance action execution remain incomplete before backend
  cutover.
- Python package exposes Runtime Core feature/version discovery, runtime
  ...
  Invocation DTOs, unary InvocationResult projection, InvocationHandle
  await/cancel/events observation, and RuntimeClient invoke/prepare/submit-signed
  methods behind narrow transport protocols; concrete daemon lifecycle/transport,
  stream/bidi adapters, profile clients, host binding bridge, EasyRemote
  extraction tests, per-profile error source refs, and conformance action
  execution remain incomplete before EasyRemote cutover.
