...
| Go | P0 | EasyNet backend/Hub | provider-backed for Runtime Core, Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, Wrappers, Access Control, and the shared conformance runner |
| Python | P0 | EasyRemote | provider-backed for Runtime Core, Directory + Identity, Receipt, Publication, Host Binding, Mission, Admin + Gateway, Events, Surface, Compatibility, Wrappers, and the shared conformance runner; seam for Access Control |
...
| Java/JVM | P1 | enterprise and Android-adjacent integrations | seam as a Maven package for Runtime Core typed errors, feature discovery, complete Invocation drafts, prepared/signed Invocation material over injected runtime transport, authority metadata projection and transport minting guardrails, `CompletableFuture` async adapters, iterator-backed bounded stream/bidi state, Health DTO/client state, Directory + Identity request/projection/subscription state, Receipt request/ref/projection/verification/chain state, Publication carrier/resource-ref state, Host Binding codec/hash/lifecycle state, Mission carrier/status/event state, Admin + Gateway carrier/status/session state, Events carrier/frame/stream state, Surface page/manifest/health state, Compatibility carrier/projection state, and Wrappers record projection state; provider transports and stable release support unsupported |
| Swift | P1 | macOS/iOS-adjacent clients | seam as a Swift Package Manager package for Runtime Core typed errors, feature discovery, complete Invocation drafts, prepared/signed Invocation material over injected runtime transport, authority metadata projection and transport minting guardrails, `AsyncSequence` bounded stream/bidi state, Health DTO/client state, Directory + Identity request/projection/subscription state, Receipt request/ref/projection/verification/chain state, Publication carrier/resource-ref state, Host Binding codec/hash/lifecycle state, Mission carrier/status/event state, Admin + Gateway carrier/status/session state, Events carrier/frame/stream state, Surface page/manifest/health state, Compatibility carrier/projection state, and Wrappers record projection state; provider transports and stable release support unsupported |
...
| receipt | Receipt | provider-backed | provider-backed | Go/Python opaque `ReceiptRef`/`ReceiptChain` anchors, RFC-007 receipt body URA construction, Axon-backed single-receipt verification, cross-invocation chain signature, and parent-receipt DAG closure verification exist for full Axon audit bundles; product receipt storage policy and external product cutover remain outside the daemon SDK facade. |
...
| access control | Access Control | provider-backed | seam | Go has a Runtime Core-backed access-control transport for RFC-014 system abilities; Python exposes the same typed DTO/client seam but still needs a Runtime Core-backed provider before provider-backed parity. |
...
- Receipt fetch carrier, projection, causal-ref guardrails,
  ...
  read models through the Receipt facade and C ABI Runtime Core invoke. Java
  and Swift now expose injected-transport receipt projection, verification, and
  chain-verification seams without local receipt URA construction. RFC-007
  receipt URA construction and broader provider cutovers remain incomplete.
...
- Admin + Gateway carrier/status guardrails exist for Rust/C ABI over daemon
  ...
  also cover device-session create/list/delete projections, while
  Node/Java/Swift expose injected-transport Admin + Gateway carrier/status/
...
- Node/TypeScript now exposes Runtime Core, Health, Authority, Directory + Identity,
  ...
  model/chat/file carrier and projection seams, WrapperClient file/terminal/
