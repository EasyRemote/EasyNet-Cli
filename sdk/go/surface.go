...
func (f SurfaceTransportFunc) BuildListPagesInvocation(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.BuildListPagesInvocationFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface list-pages invocation transport function is required")
	}
...
func (f SurfaceTransportFunc) BuildCreatePageInvocation(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.BuildCreatePageInvocationFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface create-page invocation transport function is required")
	}
...
func (f SurfaceTransportFunc) BuildDeletePageInvocation(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.BuildDeletePageInvocationFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface delete-page invocation transport function is required")
	}
...
func (f SurfaceTransportFunc) BuildManifestInvocation(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.BuildManifestInvocationFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface manifest invocation transport function is required")
	}
...
func (f SurfaceTransportFunc) BuildHealthInvocation(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.BuildHealthInvocationFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface health invocation transport function is required")
	}
...
func (f SurfaceTransportFunc) ListPages(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.ListPagesFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface list-pages transport function is required")
	}
...
func (f SurfaceTransportFunc) CreatePage(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.CreatePageFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface create-page transport function is required")
	}
...
func (f SurfaceTransportFunc) DeletePage(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.DeletePageFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface delete-page transport function is required")
	}
...
func (f SurfaceTransportFunc) SurfaceManifest(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.SurfaceManifestFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface manifest transport function is required")
	}
...
func (f SurfaceTransportFunc) PublicPageRef(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.PublicPageRefFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface public-page-ref transport function is required")
	}
...
func (f SurfaceTransportFunc) SurfaceHealth(ctx context.Context, requestJSON []byte) ([]byte, error) {
	if f.SurfaceHealthFunc == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface health transport function is required")
	}
...
func NewSurfaceClient(transport SurfaceTransport) (*SurfaceClient, error) {
	if transport == nil {
		return nil, invalidProfileClient(surfaceProfile, "surface transport is required")
	}
...
func (c *SurfaceClient) PublicPageRef(ctx context.Context, req SurfacePublicPageRefRequest) (SurfacePublicPageRef, error) {
	...
	if err != nil {
		return SurfacePublicPageRef{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("encode surface public ref request: %v", err), err)
	}
...
func (c *SurfaceClient) Close(ctx context.Context) error {
	if c == nil || c.transport == nil {
		return invalidProfileClient(surfaceProfile, "surface client is not initialized")
	}
...
func NewSurfacePageRecordFromJSON(raw []byte) (SurfacePageRecord, error) {
	...
	if err := json.Unmarshal(raw, &record); err != nil {
		return SurfacePageRecord{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface page record JSON: %v", err), err)
	}
...
func NewSurfacePagePageFromJSON(raw []byte) (SurfacePagePage, error) {
	...
	if err := json.Unmarshal(raw, &page); err != nil {
		return SurfacePagePage{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface page page JSON: %v", err), err)
	}
	if page.Profile != surfaceProfile || page.Kind != "surface_page_page" ||
		...
		return SurfacePagePage{}, invalidProfilePayload(surfaceProfile, "invalid surface page projection", nil)
	}
	if page.Limit < 1 || page.Limit > MaxSurfacePageSize {
		return SurfacePagePage{}, invalidProfilePayload(surfaceProfile, "surface page limit exceeds bounds", nil)
	}
...
func NewSurfaceManifestFromJSON(raw []byte) (SurfaceManifest, error) {
	...
	if err := json.Unmarshal(raw, &manifest); err != nil {
		return SurfaceManifest{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface manifest JSON: %v", err), err)
	}
	if manifest.Profile != surfaceProfile || manifest.Kind != "surface_manifest" ||
		...
		return SurfaceManifest{}, invalidProfilePayload(surfaceProfile, "invalid surface manifest projection", nil)
	}
...
func NewSurfacePublicPageRefFromJSON(raw []byte) (SurfacePublicPageRef, error) {
	...
	if err := json.Unmarshal(raw, &ref); err != nil {
		return SurfacePublicPageRef{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface public page ref JSON: %v", err), err)
	}
	if ref.Profile != surfaceProfile || ref.Kind != "public_page_ref" || ref.PageID == "" ||
		...
		return SurfacePublicPageRef{}, invalidProfilePayload(surfaceProfile, "invalid surface public page ref projection", nil)
	}
	return ref, nil
}
...
func NewSurfaceMutationResultFromJSON(raw []byte) (SurfaceMutationResult, error) {
	...
	if err := json.Unmarshal(raw, &result); err != nil {
		return SurfaceMutationResult{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface mutation result JSON: %v", err), err)
	}
	if result.Profile != surfaceProfile || result.Kind != "surface_mutation_result" ||
		...
		return SurfaceMutationResult{}, invalidProfilePayload(surfaceProfile, "invalid surface mutation result projection", nil)
	}
	if result.State != "deleted" && result.State != "unknown" {
		return SurfaceMutationResult{}, invalidProfilePayload(surfaceProfile, "invalid surface mutation state", nil)
	}
	return result, nil
}
...
func NewSurfaceHealthFromJSON(raw []byte) (SurfaceHealth, error) {
	...
	if err := json.Unmarshal(raw, &dto); err != nil {
		return SurfaceHealth{}, invalidProfilePayload(surfaceProfile, fmt.Sprintf("decode surface health JSON: %v", err), err)
	}
	...
	for _, check := range dto.Checks {
		if check.LatencyMS == nil {
			return SurfaceHealth{}, invalidProfilePayload(surfaceProfile, "invalid surface health check projection", nil)
		}
	...
	if health.Profile != surfaceProfile || health.Kind != "surface_health" || health.State == "" ||
		...
		health.Metadata == nil {
		return SurfaceHealth{}, invalidProfilePayload(surfaceProfile, "invalid surface health projection", nil)
	}
...
func marshalSurfaceRequest(req any, validate func(any) error) ([]byte, error) {
	...
	if err != nil {
		return nil, invalidProfilePayload(surfaceProfile, fmt.Sprintf("encode surface request: %v", err), err)
	}
...
func validateSurfaceListPagesRequest(req any) error {
	...
	if limit < 1 || limit > MaxSurfacePageSize {
		return invalidProfilePayload(surfaceProfile, "surface page limit exceeds bounds", nil)
	}
	return nil
}
...
func validateSurfaceCreatePageRequest(req any) error {
	...
	if value.Folder == "" || !strings.HasPrefix(value.Folder, "/") {
		return invalidProfilePayload(surfaceProfile, "surface folder must be absolute", nil)
	}
	if value.Visibility != "" && value.Visibility != "public" && value.Visibility != "private" {
		return invalidProfilePayload(surfaceProfile, "invalid surface visibility", nil)
	}
	return nil
}
...
func validateSurfaceCarrierBase(base SurfaceCarrierBase) error {
	if base.CallerURA == "" || base.CalleeURA == "" || base.SubjectURA == "" ||
		...
		return invalidProfilePayload(surfaceProfile, "complete surface invocation carrier is required", nil)
	}
	return nil
}
...
func validateSurfaceRef(value string) error {
	if strings.TrimSpace(value) == "" || strings.TrimSpace(value) != value {
		return invalidProfilePayload(surfaceProfile, "surface_ref must be a clean daemon ref", nil)
	}
	if strings.HasPrefix(value, "http://") || strings.HasPrefix(value, "https://") {
		return invalidProfilePayload(surfaceProfile, "surface_ref must not be an HTTP route", nil)
	}
	if !strings.HasPrefix(value, "easynet://") {
		return invalidProfilePayload(surfaceProfile, "surface_ref must be an EasyNet ref", nil)
	}
	return nil
}
...
func validateSurfaceProjectID(value string) error {
	if value == "" || len(value) > 64 {
		return invalidProfilePayload(surfaceProfile, "invalid surface project_id", nil)
	}
...
func validateSurfacePageRecord(record SurfacePageRecord) error {
	if record.Profile != surfaceProfile || record.Kind != "page_record" || record.PageID == "" ||
		...
		return invalidProfilePayload(surfaceProfile, "invalid surface page record projection", nil)
	}
	return nil
}
...
func wrapSurfaceTransportError(message string, cause error) error {
	...
	if errors.As(cause, &sdkErr) {
		return withProfileErrorDetails(sdkErr, surfaceProfile)
	}
	return transportProfileError(surfaceProfile, message, cause)
}
