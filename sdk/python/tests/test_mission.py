...
class MissionTests(unittest.TestCase):
    def test_easyremote_pipeline_plan_renders_eal_and_child_intents(self) -> None:
        plan = EasyRemotePipelinePlan(
            "nightly-report",
        ...
        self.assertIn("// generated by easyremote 0.9.0", eal)
    ...
    def test_easyremote_pipeline_plan_rejects_invalid_fields(self) -> None:
        plan = EasyRemotePipelinePlan("p")
        with self.assertRaises(SDKError):
            plan.step("er.fn", timeout=0)
        with self.assertRaises(SDKError):
            plan.step("er.fn", args={"payload": {"nested": 1}})

        foreign = EasyRemotePipelinePlan("other").step("er.src")
        with self.assertRaises(SDKError) as caught:
            plan.step("er.fn", args={"data": foreign.output})
        self.assertIn("not part of this pipeline", caught.exception.message)

    def test_easyremote_pipeline_plan_validates_child_invocation_facts(self) -> None:
        plan = EasyRemotePipelinePlan("nightly")
        ...
        missing = EasyRemotePipelinePlan("nightly")
        ...
        self.assertEqual(
            ...
            "pipeline_child_invocation_mismatch",
        )
