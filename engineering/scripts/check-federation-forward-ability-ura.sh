...
[[ -f src/daemon/invocation/federation_invoke.rs ]] || fail "missing src/daemon/invocation/federation_invoke.rs"
...
bad_ability_ura_calls="$(
    find src tests -name '*.rs' -print 2>/dev/null \
        | grep -v 'src/daemon/invocation/federation_invoke.rs' \
        | sort \
...
grep -q 'struct RemoteAbilityInvocationTarget' src/daemon/invocation/federation_invoke.rs \
    || fail "missing explicit remote ability invocation target value object"

if grep -q 'TargetOwnedAbilityUra' src/daemon/invocation/federation_invoke.rs; then
    fail "old target-owned Ability URA compatibility type must stay retired"
fi

grep -q 'fn invoke_via_federation_forward_target' src/daemon/invocation/federation_invoke.rs \
