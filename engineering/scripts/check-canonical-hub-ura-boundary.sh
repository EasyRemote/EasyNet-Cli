...
require_file src/daemon/invocation/federation_invoke.rs
require_file src/daemon/invocation/admission_facade.rs
require_file src/daemon/invocation/daemon_invocation_service.rs
require_file src/daemon/invocation/register_device_pubkey.rs
...
require_any_grep src/daemon/invocation/federation_invoke.rs \
    "parse_node_ura must delegate grammar to Axon" \
    'use easynet_axon::ura::{parse_ura, URAKind};' \
    'use easynet_axon::ura::{URAKind, parse_ura};'
require_grep 'whose clean protocol identity is `easynet:///r/<realm>/hub`' src/daemon/invocation/federation_invoke.rs \
    "parse_node_ura docs must state the canonical /hub identity"
require_grep 'fn parse_node_ura_accepts_protocol_hub_identity()' src/daemon/invocation/federation_invoke.rs \
    "parse_node_ura tests must accept literal canonical /hub"
require_grep 'fn parse_node_ura_rejects_hub_with_tail()' src/daemon/invocation/federation_invoke.rs \
    "parse_node_ura tests must reject Hub URAs with tail"
require_grep 'parse_node_ura("easynet:///r/realm/hub/extra")' src/daemon/invocation/federation_invoke.rs \
...
require_grep 'assert!(facade.is_federated_caller("easynet:///r/peer-realm/hub"));' \
    src/daemon/invocation/admission_facade.rs \
...
require_grep 'parse_realm_from_ura("easynet:///r/abc/hub")' \
    src/daemon/invocation/register_device_pubkey.rs \
