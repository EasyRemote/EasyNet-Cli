...
require_path "src/daemon/ability/builtins/mod.rs"
require_path "src/daemon/ability/catalog/mod.rs"
...
scan_must_be_empty \
    "active code must not import through retired runtime::local_invocation_identity paths" \
    '(^|[^[:alnum:]_])(crate::runtime::local_invocation_identity::|easynet_cli::runtime::local_invocation_identity::|runtime::local_invocation_identity::)' \
    "${SCAN_ROOTS[@]}"

scan_must_be_empty \
    "active code must not import through retired runtime::invocation_target paths" \
    '(^|[^[:alnum:]_])(crate::runtime::invocation_target::|easynet_cli::runtime::invocation_target::|runtime::invocation_target::)' \
    "${SCAN_ROOTS[@]}"
...
scan_must_be_empty \
    "active code must not import through retired runtime::owner_projection paths" \
    '(^|[^[:alnum:]_])(crate::runtime::owner_projection::|easynet_cli::runtime::owner_projection::|runtime::owner_projection::)' \
    "${SCAN_ROOTS[@]}"
...
scan_must_be_empty \
    "active code must not import through retired runtime::system_ability_catalog paths" \
    '(^|[^[:alnum:]_])(crate::runtime::system_ability_catalog::|easynet_cli::runtime::system_ability_catalog::|runtime::system_ability_catalog::)' \
    "${SCAN_ROOTS[@]}"
...
scan_must_be_empty \
    "active code must not reference retired src/runtime/invocation_target.rs physical path" \
    'src/runtime/invocation_target\.rs|runtime/invocation_target\.rs' \
    "${SCAN_ROOTS[@]}"
...
scan_must_be_empty \
    "active code must not reference retired src/runtime/system_abilities physical path" \
    'src/runtime/system_abilities(/|$)' \
    "${SCAN_ROOTS[@]}"

scan_must_be_empty \
    "active code must not reference retired src/runtime/system_ability_catalog physical path" \
    'src/runtime/system_ability_catalog(/|$)' \
    "${SCAN_ROOTS[@]}"
