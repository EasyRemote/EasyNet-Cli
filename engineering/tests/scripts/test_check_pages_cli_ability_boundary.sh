...
SCRIPT="$REPO_ROOT/engineering/scripts/check-pages-cli-ability-boundary.sh"
...
make_sandbox() {
    ...
    cp "$REPO_ROOT/src/cli/pages.rs" "$sandbox/src/cli/pages.rs"
    echo "$sandbox"
}
...
SB="$(make_sandbox)"
perl -0pi -e 's/enum PagesAbilityVerb/enum PageVerb/' "$SB/src/cli/pages.rs"
rc=0
...
SB="$(make_sandbox)"
perl -0pi -e 's/let ability = PagesAbility::for_user\(&user, PagesAbilityVerb::Publish\)\?;/let ability = format!("{user}.pages.publish");/' "$SB/src/cli/pages.rs"
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
rm -rf "$SB"
