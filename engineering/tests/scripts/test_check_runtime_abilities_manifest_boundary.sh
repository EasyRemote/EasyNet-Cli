...
SCRIPT="$REPO_ROOT/engineering/scripts/check-runtime-abilities-manifest-boundary.sh"
...
mkdir -p "$SB/engineering/scripts" "$SB/src/runtime"
cp "$SCRIPT" "$SB/engineering/scripts/check-runtime-abilities-manifest-boundary.sh"
cat > "$SB/src/runtime/agent_ability_specs.rs" <<'RS'
...
(
  cd "$SB"
  bash engineering/scripts/check-runtime-abilities-manifest-boundary.sh
