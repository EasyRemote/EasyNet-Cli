...
SCRIPT="$REPO_ROOT/engineering/scripts/check-credentials-username-contract.sh"
...
make_sandbox() {
    local sandbox
    sandbox="$(mktemp -d)"
    mkdir -p "$sandbox/src/persistence" "$sandbox/src/cli"
    cp "$REPO_ROOT/src/persistence/config.rs" "$sandbox/src/persistence/config.rs"
    cp "$REPO_ROOT/src/cli/join.rs" "$sandbox/src/cli/join.rs"
    cp "$REPO_ROOT/src/cli/start.rs" "$sandbox/src/cli/start.rs"
    echo "$sandbox"
}
...
SB="$(make_sandbox)"
cat >>"$SB/src/cli/join.rs" <<'RS'

fn backfill_credentials_username_from_auth_session() {
    let _ = crate::cli::auth::load_session();
}
RS
rc=0
