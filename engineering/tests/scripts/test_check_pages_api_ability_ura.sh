#!/usr/bin/env bash
#
# Contract tests for scripts/check-pages-api-ability-ura.sh.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SCRIPT="$REPO_ROOT/engineering/scripts/check-pages-api-ability-ura.sh"
...
make_sandbox() {
    local sandbox
    sandbox="$(mktemp -d)"
    mkdir -p "$sandbox/src/runtime/system_abilities/resources/pages" "$sandbox/docs"
    cp "$REPO_ROOT/src/runtime/system_abilities/resources/pages/api.rs" "$sandbox/src/runtime/system_abilities/resources/pages/api.rs"
...
SB="$(make_sandbox)"
perl -0pi -e 's/#\[serde\(deny_unknown_fields\)\]\n//' "$SB/src/runtime/system_abilities/resources/pages/api.rs"
rc=0
...
SB="$(make_sandbox)"
perl -0pi -e 's/ability_ura: Option<String>/ability: Option<String>/' "$SB/src/runtime/system_abilities/resources/pages/api.rs"
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
rm -rf "$SB"
...
SB="$(make_sandbox)"
perl -0pi -e 's/AbilitySelector::parse\(&ability_ura\)/crate::ura::parse_ura(&ability_ura)/' "$SB/src/runtime/system_abilities/resources/pages/api.rs"
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
rm -rf "$SB"
