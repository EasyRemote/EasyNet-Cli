...
SCRIPT="$REPO_ROOT/engineering/scripts/check-federation-forward-ability-ura.sh"
...
make_sandbox() {
    local sandbox
    sandbox="$(mktemp -d)"
    mkdir -p "$sandbox/src/daemon/invocation" "$sandbox/tests"
...
SB="$(make_sandbox)"
cat >>"$SB/src/daemon/invocation/federation_invoke.rs" <<'RS'
...
pub fn probe() {
    crate::daemon::invocation::federation_invoke::invoke_via_federation_forward();
}
RS
rc=0
run_check "$SB" >/dev/null 2>&1 || rc=$?
...
cat >"$SB/src/cli/probe.rs" <<'RS'
pub fn probe() {
    crate::daemon::invocation::federation_invoke::invoke_via_federation_forward_ability_ura(
